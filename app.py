...
def init_db():
    ...
    if REVIEWER_EMAIL and REVIEWER_PASSWORD:
        try:
            conn.execute("""INSERT OR REPLACE INTO users (email,role,voice_gender,password_hash,code_expiry,created_at) VALUES (?, 'reviewer', 'male', ?, '2099-12-31T23:59:59', ?)""",(REVIEWER_EMAIL.lower(),generate_password_hash(REVIEWER_PASSWORD),datetime.now().isoformat()))
            print(f"✅ حساب المراجع جاهز: {REVIEWER_EMAIL}")
        except Exception as e:
            print(f"❌ خطأ في إعداد حساب المراجع: {e}")
