...
def shared_conversation(cid):
    ...
    if r:
        ...
        return render_template_string(SPH, messages=m, title=t)
...
def load_conversation(cid):
    uid = get_user_id()
    ms = load_conversation_by_id(uid, cid)
    if ms:
...
def set_gender():
    d = request.get_json()
    session['voice_gender'] = d.get('gender', 'male')
    return jsonify({"status": "ok"})
...
def chat():
    try:
        ...
        # ====== تعديل 1: زيادة عدد الرسائل المستخدمة في السياق إلى 30 ======
        ch = server_hist[-30:]
        ...
        # ====== الرد من OpenAI مع تفعيل مميزات GPT-5.6 Luna ======
        try:
            # ====== تعديل 2: تحديد مستوى التفكير حسب الصلاحية ======
            reasoning_level = "low" if not is_admin else "high"

            r = client.chat.completions.create(
                model=model,
                messages=msgs,
                # ====== تعديل 3: رفع عدد الرموز إلى 8000 ======
                max_completion_tokens=8000,
                temperature=0.8,
                reasoning_effort=reasoning_level  # تفعيل خاصية التفكير العميق
            )
