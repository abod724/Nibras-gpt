...
# ========== نظام التعليمات الجديد (مع إجبار القوائم الأفقية) ==========
SP=f"""
أنت "نبراس"، مساعد شخصي ذكي تتحدث باللهجة العامية البيضاء.

**مصادر معرفتك:**
١. **ملف المعرفة** (أدناه) هو مرجعك الأساسي.
٢. **معرفتك العامة**.
٣. **البحث بالويب** تستخدمه عندما يسألك عن أي شيء حديث أو غير موجود في ملف المعرفة.

**ملف المعرفة الخاص بك:**
{kc}

**⚠️ قواعد التنسيق الإلزامية (طبقها في كل رد):**
- اكتب ردك باستخدام لغة **Markdown** تماماً مثل التنسيق التالي:
  - استخدم `#` للعنوان الرئيسي.
  - استخدم `##` للعناوين الفرعية.
  - استخدم `---` كخط فاصل بين الأقسام.
  - استخدم `**النص**` للكتابة العريضة.
- **ممنوع منعاً باتاً** كتابة النقاط أو الأرقام في سطور منفصلة (ممنوع القوائم العمودية).
- إذا أردت سرد خطوات أو نقاط، اكتبها في **سطر واحد طويل** من اليمين لليسار، واستخدم الأرقام العربية (١، ٢، ٣) أو الإنجليزية (1, 2, 3) مفصولة بفواصل أو مسافات.
  - **مثال صحيح (التزم به):** "١. حمص البن، ٢. اطحنه، ٣. اغلي الماء، ٤. صبّه في الفنجان."
- اترك سطراً فارغاً بين الفقرات لتسهيل القراءة.
- حافظ على لهجتك العامية البيضاء.
- إذا لم تجد المعلومة، قل "ما عندي علم".
- لا تكتب "لحظة" أو "انتظر"، فقط رد مباشرة.
"""
# ==================================================================
...
function buildTable(rows) {
    ...
    var separator = splitRow(rows[1]);
    // نتأكد أن الصف الثاني فعلاً صف تنسيق Markdown
    var isSeparator = separator.length > 0 && separator.every(function(cell) {
        return /^:?-{3,}:?$/.test(cell);
    });
    if (!isSeparator) {
    ...
    var align = separator.map(function(cell) {
        if (cell.startsWith(':') && cell.endsWith(':')) return 'center';
        if (cell.endsWith(':')) return 'right';
        return 'left';
    });
    var html = '<div style="overflow-x:auto;margin:12px 0;">';
    html += '<table style="border-collapse:collapse;width:100%;font-size:.95em;min-width:300px;">';
    html += '<thead><tr>';
    header.forEach(function(cell, i) {
        html += '<th style="' +
            'border:1px solid var(--border-color);' +
            'padding:9px;' +
            'text-align:' + (align[i] || 'right') + ';' +
            'background:var(--bg-input);' +
            'font-weight:700;' +
            '">' + formatInlineMarkdown(cell) + '</th>';
    });
    html += '</tr></thead><tbody>';
...
function formatInlineMarkdown(text) {
    ...
    s = s.replace(
        /!\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)/g,
        '<img src="$2" alt="$1" style="max-width:100%;border-radius:10px;margin:8px 0;display:block;" loading="lazy">'
    );
    // الروابط
    s = s.replace(
        /\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g,
    ...
    s = s.replace(
        /`([^`\n]+)`/g,
        '<code style="background:var(--bg-input);padding:2px 6px;border-radius:5px;font-family:monospace;font-size:.9em;">$1</code>'
    );
    // خط أفقي
    s = s.replace(
        /^(?:---|\*\*\*|___)\s*$/gm,
...
function formatBotText(t) {
    ...
    // استخراج كتل الأكواد قبل أي معالجة Markdown
    var codeBlocks = [];
    text = text.replace(
        /```([a-zA-Z0-9_+-]*)\n?([\s\S]*?)```/g,
    ...
    function flushParagraph() {
        ...
        if (content) {
            output.push(
                ...
                formatInlineMarkdown(content).replace(/\n/g, '<br>') +
                '</p>'
            );
        }
        paragraph = [];
    ...
    function flushList() {
        ...
        var style = tag === 'ol' ?
            'margin:8px 0;padding-right:24px;' :
            'margin:8px 0;padding-right:24px;';
        output.push(
            '<' + tag + ' style="' + style + '">' +
            listItems.join('') +
    ...
    function flushQuote() {
        ...
        var quote = quoteLines
            .map(function(line) { return formatInlineMarkdown(line); })
            .join('<br>');
        output.push(
            '<blockquote style="' +
    ...
    function flushTable() {
        ...
        output.push(buildTable(tableRows));
        tableRows = [];
    }
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (codeMatch) {
            ...
            output.push(
                ...
                'margin:10px 0;' +
                'border:1px solid var(--border-color);' +
                '"><code>' + langLabel + block.code + '</code></pre>'
            );
            continue;
        }
        ...
        if (heading) {
            ...
            var sizes = { 1: '1.6em', 2: '1.4em', 3: '1.25em', 4: '1.15em', 5: '1.05em', 6: '1em' };
            output.push(
                '<h' + level + ' style="' +
                'margin:18px 0 8px;' +
                ...
                'font-weight:700;' +
                'line-height:1.5;' +
                '">' + formatInlineMarkdown(heading[2]) + '</h' + level + '>'
            );
            continue;
        }
        ...
        if (ordered) {
            ...
            listItems.push(
                ...
                formatInlineMarkdown(ordered[1]) +
                '</li>'
            );
            continue;
        }
        ...
        if (unordered) {
            ...
            listItems.push(
                '<li style="margin:4px 0;">' +
                formatInlineMarkdown(unordered[1]) +
                '</li>'
            );
            continue;
        }
