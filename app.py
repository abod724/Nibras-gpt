...
function formatBotText(t){let s=String(t||'');const codeBlocks=[];s=s.replace(/```(\w*)\n([\s\S]*?)```/g,function(m,lang,code){const idx=codeBlocks.length;codeBlocks.push({lang:lang||'',code:code.replace(/\s+$/,'')});return '\u0000CODE'+idx+'\u0000';});s=s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');let paragraphs=s.split(/\n\s*\n/);let result=paragraphs.map(function(p){if(p.indexOf('\u0000CODE')!==-1){return p.split('\n').map(function(line){if(line.indexOf('\u0000CODE')!==-1)return line;return line.trim();}).filter(function(l){return l.length>0}).join('<br>');}return p.replace(/[\r\n]+/g,' ').trim();}).filter(function(p){return p.length>0}).join('<br><br>');result=result.replace(/\u0000CODE(\d+)\u0000/g,function(m,idx){const b=codeBlocks[parseInt(idx)];if(!b)return '';const safeCode=b.code.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');return '<div class="code-block-wrapper"><button class="copy-code-btn" data-code="'+b.code.replace(/"/g,'&quot;')+'">📋 نسخ الكود</button><pre><code>'+safeCode+'</code></pre></div>';});return result;}
