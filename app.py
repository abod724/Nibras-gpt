...
def generate_image(prompt):
    try:
        ...
        if not access_key:return "ERROR: UNSPLASH_ACCESS_KEY غير موجود في البيئة"
        ...
        data = response.json()
        if response.status_code == 200 and data.get("urls"):return data["urls"]["regular"]
        else:
 ...
 try:
  ...
  def is_image_request(text):
      ...
      for phrase in draw_phrases:
          if text_lower.startswith(phrase) or phrase in text_lower:return True
      return False
