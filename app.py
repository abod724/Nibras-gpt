...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Arial, sans-serif; }
        ...
        /* أنماط الماركداون (لإظهار أحجام العناوين والقوائم) */
        .msg .bot-content h1, .msg .bot-content h2, .msg .bot-content h3 { display: block; margin: 15px 0 10px 0; }
        .msg .bot-content h1 { font-size: 26px; font-weight: 800; }
        .msg .bot-content h2 { font-size: 22px; font-weight: 800; }
        .msg .bot-content h3 { font-size: 18px; font-weight: 700; }
...
<script>
    (function() {
        ...
        // ===== الدالة الحاسمة: الحفاظ على الماركداون مع دمج الفقرات =====
        function formatBotText(text) {
            ...
            // 1. تحويل العناوين الكبيرة والصغيرة
            ...
            // 4. تقسيم النص إلى فقرات (الأسطر الفارغة المزدوجة)
            var paragraphs = safe.split(/\n\s*\n/);

            // 5. دمج الأسطر المفردة داخل الفقرة، ولكن دون كسر العناوين أو القوائم!
            return paragraphs.map(function(paragraph) {
                paragraph = paragraph.trim();
                if (!paragraph) return '';
                // إذا كانت الفقرة تحتوي على HTML (عنوان أو قائمة)، اتركها كما هي
                if (paragraph.startsWith('<h') || paragraph.startsWith('<ul') || paragraph.startsWith('<ol')) {
                    return paragraph;
                }
                // وإلا، قم بدمج الأسطر لتصبح فقرة متصلة
