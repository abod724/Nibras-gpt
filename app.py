...
**⚠️ تعليمات التنسيق الإلزامية (يجب الالتزام بها):**
- استخدم **Markdown** في ردودك لجعلها مقروءة وجذابة.
- للعناوين استخدم `#` (مثلاً `# عنوان رئيسي`، `## عنوان فرعي`، `### عنوان فرعي صغير`).
- للقوائم استخدم `-` أو `*` (مثلاً `- عنصر أول`).
- للخط العريض استخدم `**نص**`.
- للخط المائل استخدم `*نص*`.
- للروابط استخدم `[النص](الرابط)`.
- للصور استخدم `![الوصف](الرابط)`.
- للاقتباسات استخدم `> نص مقتبس`.
- للجداول استخدم التنسيق التالي:
  | العمود1 | العمود2 |
  |---------|---------|
  | قيمة1   | قيمة2   |
...
# ================== HT المعدل (مع دالة formatBotText المحسّنة) ==================
...
// ================== دالة formatBotText المحسّنة (تدعم Markdown كاملاً) ==================
function buildTable(rows) {
  if (rows.length < 2) return rows.join('<br>');
  const header = rows[0].split('|').map(c => c.trim()).filter(c => c);
  const alignRow = rows[1].split('|').map(c => c.trim()).filter(c => c);
  ...
  let html = '<table style="border-collapse:collapse;width:100%;margin:10px 0;font-size:0.95em;">';
  html += '<thead><tr>';
  header.forEach((cell, i) => {
    html += `<th style="border:1px solid #ddd;padding:8px;text-align:${align[i]||'left'};background:#f5f7fa;">${cell}</th>`;
  });
  html += '</tr></thead><tbody>';
  dataRows.forEach(row => {
    ...
    cells.forEach((cell, i) => {
      html += `<td style="border:1px solid #ddd;padding:8px;text-align:${align[i]||'left'};">${cell}</td>`;
    });
    html += '</tr>';
  });
  html += '</tbody></table>';
...
function formatBotText(t) {
  ...
  // 1. الصور ![alt](url)
  s = s.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" style="max-width:100%;border-radius:8px;margin:8px 0;">');
  
  // 2. الروابط [text](url)
  s = s.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank" rel="noopener">$1</a>');
  
  // 3. العناوين
  s = s.replace(/^### (.*)$/gm, '<h3 style="margin:12px 0 6px;font-size:1.2em;">$1</h3>');
  s = s.replace(/^## (.*)$/gm, '<h2 style="margin:16px 0 8px;font-size:1.4em;">$1</h2>');
  s = s.replace(/^# (.*)$/gm, '<h1 style="margin:20px 0 10px;font-size:1.6em;">$1</h1>');
  
  // 4. النصوص الغامقة والمائلة
  s = s.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
  s = s.replace(/\*(.*?)\*/g, '<em>$1</em>');
  
  // 5. الكود
  s = s.replace(/`([^`]+)`/g, '<code style="background:#f0f0f0;padding:2px 6px;border-radius:4px;">$1</code>');
  
  // 6. الاقتباسات
  s = s.replace(/^> (.*)$/gm, '<blockquote style="border-right:4px solid #4a6a8a;padding:6px 16px;margin:8px 0;background:#f9f9f9;">$1</blockquote>');
  
  // 7. القوائم النقطية والرقمية (تحويل كل عنصر إلى <li>)
  s = s.replace(/^[\-\*] (.*)$/gm, '<li>$1</li>');
  s = s.replace(/^\d+\. (.*)$/gm, '<li>$1</li>');
  
  // 8. معالجة الجداول (بشرط أن تكون على شكل | ... |)
  const lines = s.split('\n');
  let inTable = false;
  let tableRows = [];
  let processedLines = [];
  ...
  // 9. تجميع عناصر القائمة المتجاورة في <ul>
  ...
  for (let line of lines2) {
    ...
    if (trimmed.startsWith('<li>')) {
      ...
      if (inList) {
        result.push('<ul style="margin:8px 0;padding-right:20px;">' + listItems.join('') + '</ul>');
        inList = false;
        listItems = [];
      }
      result.push(line);
  ...
  // 10. تقسيم إلى فقرات (مع تجاهل العناصر التي هي بالفعل HTML)
  const paragraphs = s.split(/\n\s*\n/);
  return paragraphs.map(p => {
    p = p.trim();
    if (!p) return '';
    // إذا كان يبدأ بوسم HTML، نتركه دون تغيير
    if (/^<(h[1-3]|ul|ol|blockquote|table|img|a)/.test(p)) {
      return p;
    }
    // وإلا نلفه في <p>
    p = p.replace(/\n/g, ' ');
