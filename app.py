...
function codeCopyHandler(e){e.stopPropagation();const btn=e.currentTarget;const code=btn.getAttribute('data-code')||btn.parentElement.querySelector('code')?.textContent||'';if(!code){showToast('❌ لا يوجد كود');return}function copyDone(){btn.textContent='✅ تم النسخ!';btn.classList.add('copied');showToast('✅ تم نسخ الكود!');setTimeout(function(){btn.textContent='📋 نسخ الكود';btn.classList.remove('copied')},2200);}function copyFail(){showToast('❌ فشل النسخ');}if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(code).then(copyDone).catch(function(){fallbackCopy(code,copyDone,copyFail)});}else{fallbackCopy(code,copyDone,copyFail);}}
function fallbackCopy(text,onSuccess,onFail){try{const ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.opacity='0';ta.style.left='-9999px';document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();onSuccess();}catch(e){onFail();}}
...
window.deleteMyData=function(){if(!confirm('⚠️ سيتم حذف جميع محادثاتك. متأكد؟'))return;fetch('/delete_my_data',{method:'POST',headers:{'Content-Type':'application/json'}}).then(r=>r.json()).then(d=>{if(d.status==='success'){alert('✅ تم الحذف');window.location.href='/'}else{alert('❌ فشل: '+(d.message||'خطأ'))}}).catch(e=>{alert('❌ خطأ في الاتصال')});};})();</script></body></html>"""
...
def admin_dashboard():
    ...
    invite_form="""<div class="card"><h3>🔐 توليد كود وصول</h3><p style="font-size:14px;color:#8b949e;margin-bottom:15px;">أدخل بريد المستخدم واختر المدة. الكود سيعمل مع هذا البريد فقط.</p><a href="/admin/generate" style="display:block;background:#58a6ff;color:#fff;text-decoration:none;padding:12px;border-radius:8px;text-align:center;font-weight:bold;">اذهب لصفحة التوليد</a></div>"""
    return f"""<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>لوحة تحكم نبراس</title><style>body{{font-family:'Segoe UI',Tahoma;background:#0d1117;color:#c9d1d9;padding:20px;margin:0}}.container{{max-width:600px;margin:auto}}h1{{color:#58a6ff;text-align:center}}.card{{background:#161b22;border-radius:15px;padding:15px;margin:15px 0;border:1px solid #30363d}}.stat{{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #21262d}}.stat:last-child{{border:none}}.num{{color:#58a6ff;font-weight:bold;font-size:18px}}.conv-item{{padding:10px 0;border-bottom:1px solid #21262d}}.conv-item small{{color:#8b949e;display:block;font-size:12px}}.back{{display:block;text-align:center;color:#58a6ff;text-decoration:none;margin-top:20px}}</style></head><body><div class="container"><h1>📊 لوحة تحكم نبراس</h1><div class="card"><div class="stat"><span>👥 المستخدمون</span><span class="num">{users_count}</span></div><div class="stat"><span>💬 المحادثات</span><span class="num">{total_convs}</span></div><div class="stat"><span>📅 اليوم</span><span class="num">{today_convs}</span></div></div>{invite_form}<div class="card"><h3>👥 المستخدمون المسجلون</h3>{users_html}</div><div class="card"><h3>🕒 آخر 10 محادثات</h3>{recent_html}</div><a href="/" class="back">⬅ الرئيسية</a></div></body></html>"""
...
document.getElementById('genForm').addEventListener('submit',function(e){
    ...
    fetch('/admin/generate_api?email='+encodeURIComponent(email)+'&days='+days)
        ...
        .then(function(data){
            if(data.code){
                document.getElementById('result').innerHTML='<div class="code-box">'+data.code+'</div><div class="info">للبريد: <b>'+data.email+'</b><br>ينتهي في: <b>'+data.expiry+'</b><br>انسخه وأرسله للمستخدم.</div>';
            }else{
