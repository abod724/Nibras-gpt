...
TOOLS_HTML="""<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>أدوات نبراس المجانية</title><style>body{font-family:'Segoe UI',Tahoma; background:#0f172a; color:#fff; margin:0; padding:20px}.container{max-width:900px; margin:auto}h1{text-align:center; color:#38bdf8}a{color:#38bdf8}.card{background:#1e293b; border-radius:15px; padding:20px; margin:20px 0; border:1px solid #334155}input,select,textarea{width:100%; padding:12px; margin:8px 0; border-radius:8px; border:none; background:#0f172a; color:#fff; box-sizing:border-box}button{background:#38bdf8; color:#000; padding:12px 20px; border:none; border-radius:8px; font-weight:bold; cursor:pointer; width:100%}button:hover{background:#0ea5e9}.result{background:#0f172a; padding:15px; border-radius:8px; margin-top:10px; border:1px dashed #38bdf8; word-break:break-all}.grid{display:grid; grid-template-columns:1fr 1fr; gap:15px}.subject-row{display:grid; grid-template-columns:1fr 1fr 30px; gap:10px; align-items:center; margin-bottom:8px}.subject-row input,.subject-row select{margin:0}.btn-small{background:#ef4444; color:#fff; padding:6px; border-radius:6px; width:30px; font-size:14px; text-align:center; border:none; cursor:pointer}.btn-small:hover{background:#dc2626}.add-btn{background:#22c55e; color:#000}.add-btn:hover{background:#16a34a}@media(max-width:600px){.grid,.subject-row{grid-template-columns:1fr}.subject-row .btn-small{width:100%}}.qr-img{max-width:100%;max-height:250px;border-radius:8px;margin-top:10px;border:2px solid #38bdf8}</style></head><body><div class="container"><h1>🧰 أدوات نبراس المجانية</h1><p style="text-align:center; color:#94a3b8">أدوات سريعة وذكية - مجانية للجميع</p><p style="text-align:center"><a href="/">⬅ ارجع لنبراس</a></p><div class="card"><h3>📚 1- حاسبة المعدل الجامعي (GPA)</h3><div id="subjectsContainer"><div class="subject-row"><input class="hours" type="number" placeholder="ساعات" value="3"><select class="grade"><option value="5">A+ (5)</option><option value="4.75">A (4.75)</option><option value="4.5">B+ (4.5)</option><option value="4">B (4)</option><option value="3.5">C+ (3.5)</option><option value="3">C (3)</option><option value="2.5">D+ (2.5)</option><option value="2">D (2)</option></select><button class="btn-small" onclick="window.removeSubject(this)">✕</button></div><div class="subject-row"><input class="hours" type="number" placeholder="ساعات" value="3"><select class="grade"><option value="5">A+ (5)</option><option value="4.75">A (4.75)</option><option value="4.5">B+ (4.5)</option><option value="4">B (4)</option><option value="3.5">C+ (3.5)</option><option value="3">C (3)</option><option value="2.5">D+ (2.5)</option><option value="2">D (2)</option></select><button class="btn-small" onclick="window.removeSubject(this)">✕</button></div></div><button class="add-btn" onclick="window.addSubject()">➕ إضافة مادة</button><button onclick="window.calcGPA()" style="margin-top:10px;">احسب المعدل</button><div id="gpaRes" class="result" style="display:none"></div></div><div class="card"><h3>📝 2- منشئ السيرة الذاتية ATS</h3><input id="cvName" placeholder="الاسم الكامل"><input id="cvSpec" placeholder="التخصص - مثلا: أمن سيبراني"><textarea id="cvExp" placeholder="خبراتك باختصار"></textarea><button onclick="window.makeCV()">أنشئ سيرتي</button><div id="cvRes" class="result" style="display:none"></div></div><div class="card"><h3>📱 3- مولد QR كود (رمز سريع)</h3><input id="qrInput" placeholder="أدخل الرابط أو النص... (مثال: https://example.com)"><button onclick="window.generateQR()">🎨 ولّد QR</button><div id="qrResult" class="result" style="display:none;text-align:center"><img id="qrImage" class="qr-img" src=""/><br><button class="btn-small" onclick="window.downloadQR()" style="width:auto;padding:8px 20px;background:#38bdf8;margin-top:10px;border:none;border-radius:8px;cursor:pointer">📥 تحميل الصورة</button></div></div><div class="card"><h3>💡 4- مولد أفكار مشاريع محلية</h3><select id="budget"><option value="5000">رأس مال 5 آلاف</option><option value="10000">10 آلاف</option><option value="20000">20 ألف</option><option value="50000">50 ألف</option></select><button onclick="window.genIdea()">عطني فكرة مشروع</button><div id="ideaRes" class="result" style="display:none"></div></div></div><script>
window.addSubject=function(){let c=document.getElementById('subjectsContainer');let d=document.createElement('div');d.className='subject-row';d.innerHTML='<input class="hours" type="number" placeholder="ساعات" value="3"><select class="grade"><option value="5">A+ (5)</option><option value="4.75">A (4.75)</option><option value="4.5">B+ (4.5)</option><option value="4">B (4)</option><option value="3.5">C+ (3.5)</option><option value="3">C (3)</option><option value="2.5">D+ (2.5)</option><option value="2">D (2)</option></select><button class="btn-small" onclick="window.removeSubject(this)">✕</button>';c.appendChild(d)}
window.removeSubject=function(e){if(document.querySelectorAll('.subject-row').length>2)e.parentElement.remove();else alert('لا يمكن حذف أقل من مادتين')}
window.calcGPA=function(){let rows=document.querySelectorAll('.subject-row');let totalHours=0,totalPoints=0;rows.forEach(r=>{let h=parseFloat(r.querySelector('.hours').value)||0;let g=parseFloat(r.querySelector('.grade').value)||0;totalHours+=h;totalPoints+=h*g});if(totalHours===0){alert('أدخل ساعات المواد');return}let gpa=(totalPoints/totalHours);let box=document.getElementById('gpaRes');box.style.display='block';let status=gpa>=4.5?'ممتاز 🔥':gpa>=3.5?'جيد جداً ✅':gpa>=2.5?'جيد ⚠️':'ضعيف 📉';box.innerHTML='<b style="font-size:22px;color:#38bdf8">'+gpa.toFixed(3)+'</b> / 5<br><span style="color:#94a3b8">التقدير: '+status+'</span><br><small>عدد المواد: '+rows.length+' | مجموع الساعات: '+totalHours+'</small>'}
window.makeCV=function(){let n=document.getElementById('cvName').value;let s=document.getElementById('cvSpec').value;let e=document.getElementById('cvExp').value;if(!n){alert('اكتب اسمك');return}let cv='السيرة الذاتية\nالاسم: '+n+'\nالتخصص: '+s+'\n\nالخبرات:\n'+e+'\n\nالمهارات:\n- العمل تحت الضغط\n- اللغة الإنجليزية\n- الحاسب الآلي\n\nالهدف: الحصول على وظيفة في مجال '+s+' والمساهمة في رؤية 2030';document.getElementById('cvRes').style.display='block';document.getElementById('cvRes').innerText=cv}
window.generateQR=function(){let val=document.getElementById('qrInput').value.trim();if(!val){alert('أدخل رابط أو نص');return}let img=document.getElementById('qrImage');img.src='https://api.qrserver.com/v1/create-qr-code/?size=300x300&data='+encodeURIComponent(val);document.getElementById('qrResult').style.display='block'}
window.downloadQR=function(){let img=document.getElementById('qrImage');if(!img.src){alert('ولّد QR أولاً');return}let a=document.createElement('a');a.href=img.src;a.download='QR_'+Date.now()+'.png';document.body.appendChild(a);a.click();a.remove()}
window.genIdea=function(){let b=document.getElementById('budget').value;const ideas={'5000':['متجر إلكتروني منتجات محلية (عسل، سمن)','خدمة كتابة بحوث للطلاب','تصميم سير ذاتية'],'10000':['مغسلة ملابس متنقلة','عربة فود ترك قهوة مختصة','متجر تغليف هدايا'],'20000':['مشروع دروس خصوصية أونلاين','استوديو تصوير صغير','محل اكسسوارات جوالات'],'50000':['مقهى طلابي قرب الجامعة','شركة توصيل داخلي','مركز تدريب حاسب']};let list=ideas[b];let rnd=list[Math.floor(Math.random()*list.length)];document.getElementById('ideaRes').style.display='block';document.getElementById('ideaRes').innerHTML='💡 فكرة مقترحة برأس مال '+b+' ريال:<br><b style="color:#facc15;font-size:18px">'+rnd+'</b><br><br>اسأل نبراس: "سوي لي دراسة جدوى لـ '+rnd+'"';}
</script></body></html>"""
