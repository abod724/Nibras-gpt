...
SPH="""<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>📄 محادثة نبراس</title><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css"><style>*{margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',Arial,sans-serif}body{background:#f4f7fc;display:flex;justify-content:center;align-items:center;min-height:100dvh;padding:20px}.container{max-width:700px;width:100%;background:#fff;border-radius:24px;box-shadow:0 10px 40px rgba(0,0,0,0.08);padding:30px 25px}.header{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #eaeef2;padding-bottom:15px;margin-bottom:25px}.header h1{font-size:22px;color:#1a2b3c}.header a{color:#4a6a8a;text-decoration:none;font-size:15px}.msg{display:flex;margin-bottom:18px;gap:10px}.msg .avatar{width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;flex-shrink:0;font-size:14px}.msg.user .avatar{background:#eaeef2;color:#1a2b3c}.msg.bot .avatar{background:#4a6a8a;color:#fff}.msg .content{background:#f5f7fa;padding:12px 18px;border-radius:16px;border-top-right-radius:4px;max-width:85%;line-height:1.8;color:#111;white-space:normal;word-wrap:break-word;overflow-wrap:break-word}.msg.user .content{background:#eaeef2}.msg.bot .content{background:#f5f7fa}.msg .content p{margin-bottom:8px}.msg .content p:last-child{margin-bottom:0}.msg .time{font-size:11px;color:#8b949e;margin-top:4px;display:block}.footer{text-align:center;margin-top:30px;padding-top:20px;border-top:1px solid #eaeef2;color:#8b949e;font-size:14px}.footer a{color:#4a6a8a;text-decoration:none;font-weight:700}@media(max-width:500px){.container{padding:15px}.msg .content{max-width:100%}}</style></head><body><div class="container"><div class="header"><h1>💬 {{ title or 'محادثة نبراس' }}</h1><a href="/">⬅ الرئيسية</a></div><div>{% for msg in messages %}<div class="msg {{ 'user' if msg.role == 'user' else 'bot' }}"><div class="avatar">{{ '👤' if msg.role == 'user' else '🤖' }}</div><div class="content">{{ msg.content|replace('\n','<br>')|e }}<span class="time">{{ loop.index }}. {{ 'مستخدم' if msg.role == 'user' else 'نبراس' }}</span></div></div>{% endfor %}</div><div class="footer">تمت المشاركة من <a href="/">نبراس</a> - مساعد ذكي</div></div></body></html>"""
TOOLS_HTML="""<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>أدوات نبراس المجانية</title><style>body{font-family:'Segoe UI',Tahoma;background:#0f172a;color:#fff;margin:0;padding:20px}.container{max-width:900px;margin:auto}h1{text-align:center;color:#38bdf8}a{color:#38bdf8}.card{background:#1e293b;border-radius:15px;padding:20px;margin:20px 0;border:1px solid #334155}input,select,textarea{width:100%;padding:12px;margin:8px 0;border-radius:8px;border:none;background:#0f172a;color:#fff;box-sizing:border-box}button{background:#38bdf8;color:#000;padding:12px 20px;border:none;border-radius:8px;font-weight:bold;cursor:pointer;width:100%}button:hover{background:#0ea5e9}.result{background:#0f172a;padding:15px;border-radius:8px;margin-top:10px;border:1px dashed #38bdf8;word-break:break-all}.grid{display:grid;grid-template-columns:1fr 1fr;gap:15px}.subject-row{display:grid;grid-template-columns:1fr 1fr 30px;gap:10px;align-items:center;margin-bottom:8px}.subject-row input,.subject-row select{margin:0}.btn-small{background:#ef4444;color:#fff;padding:6px;border-radius:6px;width:30px;font-size:14px;text-align:center;border:none;cursor:pointer}.btn-small:hover{background:#dc2626}.add-btn{background:#22c55e;color:#000}.add-btn:hover{background:#16a34a}@media(max-width:600px){.grid,.subject-row{grid-template-columns:1fr}.subject-row .btn-small{width:100%}}.qr-img{max-width:100%;max-height:250px;border-radius:8px;margin-top:10px;border:2px solid #38bdf8}</style></head><body><div class="container"><h1>🧰 أدوات نبراس المجانية</h1><p style="text-align:center;color:#94a3b8">أدوات سريعة وذكية - مجانية للجميع</p><p style="text-align:center"><a href="/">⬅ ارجع لنبراس</a></p><div class="card"><h3>📚 1- حاسبة المعدل الجامعي (GPA)</h3><div id="subjectsContainer"><div class="subject-row"><input class="hours" type="number" placeholder="ساعات" value="3"><select class="grade"><option value="5">A+ (5)</option><option value="4.75">A (4.75)</option><option value="4.5">B+ (4.5)</option><option value="4">B (4)</option><option value="3.5">C+ (3.5)</option><option value="3">C (3)</option><option value="2.5">D+ (2.5)</option><option value="2">D (2)</option></select><button class="btn-small remove-subject">✕</button></div><div class="subject-row"><input class="hours" type="number" placeholder="ساعات" value="3"><select class="grade"><option value="5">A+ (5)</option><option value="4.75">A (4.75)</option><option value="4.5">B+ (4.5)</option><option value="4">B (4)</option><option value="3.5">C+ (3.5)</option><option value="3">C (3)</option><option value="2.5">D+ (2.5)</option><option value="2">D (2)</option></select><button class="btn-small remove-subject">✕</button></div></div><button id="addSubjectBtn" class="add-btn">➕ إضافة مادة</button><button id="calcGpaBtn">احسب المعدل</button><div id="gpaRes" class="result" style="display:none"></div></div><div class="card"><h3>📝 2- منشئ السيرة الذاتية ATS</h3><input id="cvName" placeholder="الاسم الكامل"><input id="cvSpec" placeholder="التخصص - مثلا: أمن سيبراني"><textarea id="cvExp" placeholder="خبراتك باختصار"></textarea><button id="makeCvBtn">أنشئ سيرتي</button><div id="cvRes" class="result" style="display:none"></div></div><div class="card"><h3>📱 3- مولد QR كود (رمز سريع)</h3><input id="qrInput" placeholder="أدخل الرابط أو النص... (مثال: https://example.com)"><button id="generateQrBtn">🎨 ولّد QR</button><div id="qrResult" class="result" style="display:none;text-align:center"><img id="qrImage" class="qr-img" src=""/><br><button id="downloadQrBtn" class="btn-small" style="width:auto;padding:8px 20px;background:#38bdf8;margin-top:10px;border:none;border-radius:8px;cursor:pointer">📥 تحميل الصورة</button></div></div><div class="card"><h3>💡 4- مولد أفكار مشاريع محلية</h3><select id="budget"><option value="5000">رأس مال 5 آلاف</option><option value="10000">10 آلاف</option><option value="20000">20 ألف</option><option value="50000">50 ألف</option></select><button id="genIdeaBtn">عطني فكرة مشروع</button><div id="ideaRes" class="result" style="display:none"></div></div></div><script>
(function(){function addSubject(){var c=document.getElementById('subjectsContainer');var d=document.createElement('div');d.className='subject-row';d.innerHTML='<input class="hours" type="number" placeholder="ساعات" value="3"><select class="grade"><option value="5">A+ (5)</option><option value="4.75">A (4.75)</option><option value="4.5">B+ (4.5)</option><option value="4">B (4)</option><option value="3.5">C+ (3.5)</option><option value="3">C (3)</option><option value="2.5">D+ (2.5)</option><option value="2">D (2)</option></select><button class="btn-small remove-subject">✕</button>';c.appendChild(d);d.querySelector('.remove-subject').addEventListener('click',function(){removeSubject(this);});}
function removeSubject(el){var rows=document.querySelectorAll('.subject-row');if(rows.length>2){el.closest('.subject-row').remove();}else{alert('لا يمكن حذف أقل من مادتين');}}
function calcGPA(){var rows=document.querySelectorAll('.subject-row');var totalHours=0,totalPoints=0;rows.forEach(function(r){var h=parseFloat(r.querySelector('.hours').value)||0;var g=parseFloat(r.querySelector('.grade').value)||0;totalHours+=h;totalPoints+=h*g;});if(totalHours===0){alert('أدخل ساعات المواد');return;}var gpa=totalPoints/totalHours;var box=document.getElementById('gpaRes');box.style.display='block';var status=gpa>=4.5?'ممتاز 🔥':gpa>=3.5?'جيد جداً ✅':gpa>=2.5?'جيد ⚠️':'ضعيف 📉';box.innerHTML='<b style="font-size:22px;color:#38bdf8">'+gpa.toFixed(3)+'</b> / 5<br><span style="color:#94a3b8">التقدير: '+status+'</span><br><small>عدد المواد: '+rows.length+' | مجموع الساعات: '+totalHours+'</small>';}
function makeCV(){var n=document.getElementById('cvName').value;var s=document.getElementById('cvSpec').value;var e=document.getElementById('cvExp').value;if(!n){alert('اكتب اسمك');return;}var cv='السيرة الذاتية\nالاسم: '+n+'\nالتخصص: '+s+'\n\nالخبرات:\n'+e+'\n\nالمهارات:\n- العمل تحت الضغط\n- اللغة الإنجليزية\n- الحاسب الآلي\n\nالهدف: الحصول على وظيفة في مجال '+s+' والمساهمة في رؤية 2030';document.getElementById('cvRes').style.display='block';document.getElementById('cvRes').innerText=cv;}
function generateQR(){var val=document.getElementById('qrInput').value.trim();if(!val){alert('أدخل رابط أو نص');return;}var img=document.getElementById('qrImage');img.src='https://api.qrserver.com/v1/create-qr-code/?size=300x300&data='+encodeURIComponent(val);document.getElementById('qrResult').style.display='block';}
function downloadQR(){var img=document.getElementById('qrImage');if(!img.src){alert('ولّد QR أولاً');return;}var a=document.createElement('a');a.href=img.src;a.download='QR_'+Date.now()+'.png';document.body.appendChild(a);a.click();a.remove();}
function genIdea(){var b=document.getElementById('budget').value;var ideas={'5000':['متجر إلكتروني منتجات محلية (عسل، سمن)','خدمة كتابة بحوث للطلاب','تصميم سير ذاتية'],'10000':['مغسلة ملابس متنقلة','عربة فود ترك قهوة مختصة','متجر تغليف هدايا'],'20000':['مشروع دروس خصوصية أونلاين','استوديو تصوير صغير','محل اكسسوارات جوالات'],'50000':['مقهى طلابي قرب الجامعة','شركة توصيل داخلي','مركز تدريب حاسب']};var list=ideas[b];var rnd=list[Math.floor(Math.random()*list.length)];var res=document.getElementById('ideaRes');res.style.display='block';res.innerHTML='💡 فكرة مقترحة برأس مال '+b+' ريال:<br><b style="color:#facc15;font-size:18px">'+rnd+'</b><br><br>اسأل نبراس: "سوي لي دراسة جدوى لـ '+rnd+'"';}
function init(){document.getElementById('addSubjectBtn').addEventListener('click',addSubject);document.getElementById('calcGpaBtn').addEventListener('click',calcGPA);document.getElementById('makeCvBtn').addEventListener('click',makeCV);document.getElementById('generateQrBtn').addEventListener('click',generateQR);document.getElementById('downloadQrBtn').addEventListener('click',downloadQR);document.getElementById('genIdeaBtn').addEventListener('click',genIdea);document.querySelectorAll('.remove-subject').forEach(function(btn){btn.addEventListener('click',function(){removeSubject(this);});});}
