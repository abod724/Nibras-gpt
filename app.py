import os,secrets,json,hashlib,re,base64,logging
...
import openai,edge_tts,httpx,uvicorn,aiosqlite
from pydantic import BaseModel

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# ===== القوالب ===== (تم اختصارها لأنها طويلة ولكنها نفس المحتوى)
SPH = """<!DOCTYPE html>... (نفس المحتوى) ...</html>"""
...
# ===== دوال قاعدة البيانات =====
async def get_db(): return await aiosqlite.connect(DB_FILE)
async def init_db():
    async with await get_db() as conn:
        await conn.execute('''CREATE TABLE IF NOT EXISTS conversations (user_id TEXT, conv_id TEXT PRIMARY KEY, messages TEXT, timestamp TEXT, title TEXT)''')
        await conn.execute('''CREATE TABLE IF NOT EXISTS cache (question TEXT PRIMARY KEY, answer TEXT, created TEXT)''')
        await conn.execute('''CREATE TABLE IF NOT EXISTS guest_usage (guest_id TEXT PRIMARY KEY, count INT DEFAULT 0, date TEXT)''')
        await conn.commit()
...
async def save_cache(question: str, answer: str):
    if len(question) < 10 or len(question) > 200: return
    if len(answer) > 2000: return
    async with await get_db() as conn:
        await conn.execute("INSERT OR REPLACE INTO cache (question, answer, created) VALUES (?,?,?)", (question.strip(), answer, datetime.now().isoformat()))
        await conn.commit()

async def get_user_conversations(user_id: str) -> List[Dict[str, Any]]:
    async with await get_db() as conn:
        rows = await conn.execute("SELECT conv_id, messages, timestamp, title FROM conversations WHERE user_id=? ORDER BY timestamp DESC", (user_id,))
        results = []
        async for row in rows:
            results.append({"id": row[0], "messages": json.loads(row[1]), "timestamp": row[2], "title": row[3]})
        return results
...
async def save_user_conversation(user_id: str, messages: List[Dict], conv_id: Optional[str] = None) -> str:
    async with await get_db() as conn:
        if conv_id is None:
            ...
            await conn.execute("INSERT INTO conversations (user_id, conv_id, messages, timestamp, title) VALUES (?,?,?,?,?)", (user_id, new_id, json.dumps(messages, ensure_ascii=False), datetime.now().isoformat(), title))
            await conn.commit(); return new_id
        else:
            await conn.execute("UPDATE conversations SET messages=?, timestamp=? WHERE user_id=? AND conv_id=?", (json.dumps(messages, ensure_ascii=False), datetime.now().isoformat(), user_id, conv_id))
            await conn.commit(); return conv_id
...
if os.path.exists(".well-known"): app.mount("/.well-known", StaticFiles(directory=".well-known"), name="well-known")
...
async def index(request: Request, nibras_session: Optional[str] = Cookie(None)):
    ...
    fake_session = {}
    if user_id.startswith("admin_"): fake_session['admin_email'] = user_id.replace("admin_", "")
    elif user_id.startswith("user_"): fake_session['user_email'] = user_id.replace("user_", "")
    return HTMLResponse(content=HT.replace("{{ session.get('admin_email') or session.get('user_email') }}", fake_session.get('admin_email') or fake_session.get('user_email', '')))

@app.get("/tools", response_class=HTMLResponse)
async def tools_page(): return HTMLResponse(content=TOOLS_HTML)
...
async def login_post(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    if email == admin_email:
        if not admin_password: return HTMLResponse(content=LH.replace("{{ error }}", "خطأ: لم يتم إعداد كلمة مرور الأدمن في الخادم."))
        if secrets.compare_digest(password, admin_password):
            ...
            return response
        else: return HTMLResponse(content=LH.replace("{{ error }}", "كلمة مرور الأدمن غير صحيحة."))
    elif "@" in email:
        response = RedirectResponse(url="/", status_code=302)
        response.set_cookie(key="nibras_session", value=set_user_cookie("user_" + email), httponly=True, max_age=3600*24*30)
        return response
    else: return HTMLResponse(content=LH.replace("{{ error }}", "يرجى إدخال بريد إلكتروني صحيح."))
...
async def load_conversation(request: Request, cid: str, nibras_session: Optional[str] = Cookie(None)):
    ...
    if msgs is None: raise HTTPException(status_code=404, detail="المحادثة غير موجودة")
...
async def delete_message(data: DeleteMessageRequest, request: Request, nibras_session: Optional[str] = Cookie(None)):
    ...
    if not msgs: return JSONResponse({"status": "error", "message": "المحادثة غير موجودة"}, status_code=404)
    if data.index < 0 or data.index >= len(msgs): return JSONResponse({"status": "error", "message": "الرسالة غير موجودة"}, status_code=404)
    del msgs[data.index]
...
async def chat_endpoint(request: Request, payload: ChatRequest, nibras_session: Optional[str] = Cookie(None)):
    ...
    if not is_admin:
        if not await check_guest_limit_safe(user_id):
            ...
            if conv_id is None: conv_id = await save_user_conversation(user_id, [{"role": "user", "content": user_message}, {"role": "assistant", "content": limit_reply}])
            else:
        ...
        if cached:
            if conv_id is None: conv_id = await save_user_conversation(user_id, [{"role": "user", "content": user_message}, {"role": "assistant", "content": cached}])
            else:
                msgs = await load_conversation_by_id(user_id, conv_id)
                if msgs is None: msgs = []
                msgs.append({"role": "user", "content": user_message})
    ...
    if is_image_req and not image_data:
        if is_video_req:
            ...
            if video_url.startswith("ERROR:"): reply = f"⚠️ عذراً، ما قدرت أجيب الفيديو. السبب: {video_url.replace('ERROR:', '')}"
            else:
                reply = f"🎬 إليك الفيديو الذي طلبتـه:\n{video_url}"
                if conv_id is None: conv_id = await save_user_conversation(user_id, [{"role": "user", "content": user_message}, {"role": "assistant", "content": reply}])
                else:
                    msgs = await load_conversation_by_id(user_id, conv_id)
                    if msgs is None: msgs = []
                    msgs.append({"role": "user", "content": user_message})
            ...
            if img_url.startswith("ERROR:"): reply = f"⚠️ عذراً، ما قدرت أولد الصورة. السبب: {img_url.replace('ERROR:', '')}"
            else:
    ...
    # ===== التعديل الجديد: رسالة الصورة لغير الأدمن =====
    if image_data and not is_admin:
        reply = "📸 آسف، لا يمكنني تحليل الصور في الوقت الحالي. يمكنك استخدام خاصية توليد الصور المجانية عبر كتابة (ارسم لي) أو (ابي فيديو)."
        if conv_id is None: conv_id = await save_user_conversation(user_id, [{"role": "user", "content": user_message}, {"role": "assistant", "content": reply}])
        else:
            msgs = await load_conversation_by_id(user_id, conv_id)
            if msgs is None: msgs = []
    ...
    if image_data and is_admin:
        messages_for_api.append({"role": "user", "content": [{"type": "text", "text": user_message or "حلل هذه الصورة"}, {"type": "image_url", "image_url": {"url": image_data}}]})

    if is_admin:
        try:
            context_text = ""
            for m in messages_for_api:
                if m["role"] == "user": context_text += m["content"] + "\n"
                elif m["role"] == "assistant": context_text += "نبراس: " + m["content"] + "\n"
            sr = client.responses.create(model=OPENAI_MODEL, instructions=f"{SYSTEM_PROMPT}\n\nسياق المحادثة السابقة:\n{context_text}", input=f"ابحث في الويب عن أحدث المعلومات حول: {user_message}، وقدم لي ملخصاً مفيداً.", tools=[{"type": "web_search"}])
            search_result = sr.output_text.strip()
            if search_result: messages_for_api.append({"role": "user", "content": f"نتيجة البحث:\n{search_result}\n\nاستخدم هذه المعلومات."})
        except Exception as e: logger.warning(f"⚠️ فشل البحث: {e}")

    try:
        reasoning_level = "low" if not is_admin else "high"
        response = client.chat.completions.create(model=OPENAI_MODEL, messages=messages_for_api, max_completion_tokens=8000, reasoning_effort=reasoning_level)
