...
# ================== HT المعدل (تم استبدال دالة formatBotText فقط) ==================
...
function formatBotText(t) {
  ...
  // تحويل القوائم النقطية والرقمية إلى عناصر <li>
  s = s.replace(/^[\-\*] (.*)$/gm, '<li>$1</li>');
  s = s.replace(/^\d+\. (.*)$/gm, '<li>$1</li>');
  
  // تجميع عناصر القائمة المتجاورة في <ul>
  const lines = s.split('\n');
  let result = [];
  let inList = false;
  let listItems = [];
  ...
  // تقسيم إلى فقرات (مع تجاهل العناصر التي هي بالفعل HTML)
  const ps = s.split(/\n\s*\n/);
  return ps.map(p => {
    p = p.trim();
    if (!p) return '';
    if (/^<(h[1-3]|ul|ol|blockquote)/.test(p)) {
      return p;
    }
    p = p.replace(/\n/g, ' ');
    return '<p>' + p + '</p>';
