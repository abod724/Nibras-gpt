...
document.addEventListener('DOMContentLoaded',function(){
 ...
 const cvBtn=document.getElementById('makeCvBtn');
 const qrBtn=document.getElementById('generateQrBtn');
 const dloadBtn=document.getElementById('downloadQrBtn');
 const ideaBtn=document.getElementById('genIdeaBtn');
 const container=document.getElementById('subjectsContainer');
 const gpaRes=document.getElementById('gpaRes');
 const cvRes=document.getElementById('cvRes');
 const qrResult=document.getElementById('qrResult');
 const qrImage=document.getElementById('qrImage');
 const ideaRes=document.getElementById('ideaRes');

 function addSubject(){
  let d=document.createElement('div');
  d.className='subject-row';
  d.innerHTML='<input class="hours" type="number" placeholder="ساعات" value="3"><select class="grade"><option value="5">A+ (5)</option><option value="4.75">A (4.75)</option><option value="4.5">B+ (4.5)</option><option value="4">B (4)</option><option value="3.5">C+ (3.5)</option><option value="3">C (3)</option><option value="2.5">D+ (2.5)</option><option value="2">D (2)</option></select><button class="btn-small remove-subject">✕</button>';
  container.appendChild(d);
  attachRemoveEvents();
 }

 function attachRemoveEvents(){
  document.querySelectorAll('.remove-subject').forEach(function(btn){
   btn.removeEventListener('click',removeSubject);
 ...
 function removeSubject(e){
  ...
  if(document.querySelectorAll('.subject-row').length>2){
   row.remove();
  }else{
   alert('لا يمكن حذف أقل من مادتين');
  }
 }
 ...
 function calcGPA(){
  ...
  rows.forEach(r=>{
   let h=parseFloat(r.querySelector('.hours').value)||0;
   let g=parseFloat(r.querySelector('.grade').value)||0;
   totalHours+=h;
   totalPoints+=h*g;
  });
  if(totalHours===0){alert('أدخل ساعات المواد');return;}
  let gpa=totalPoints/totalHours;
  gpaRes.style.display='block';
  let status=gpa>=4.5?'ممتاز 🔥':gpa>=3.5?'جيد جداً ✅':gpa>=2.5?'جيد ⚠️':'ضعيف 📉';
  gpaRes.innerHTML='<b style="font-size:22px;color:#38bdf8">'+gpa.toFixed(3)+'</b> / 5<br><span style="color:#94a3b8">التقدير: '+status+'</span><br><small>عدد المواد: '+rows.length+' | مجموع الساعات: '+totalHours+'</small>';
 }

 function makeCV(){
  let n=document.getElementById('cvName').value;
  let s=document.getElementById('cvSpec').value;
  let e=document.getElementById('cvExp').value;
  if(!n){alert('اكتب اسمك');return;}
  let cv='السيرة الذاتية\nالاسم: '+n+'\nالتخصص: '+s+'\n\nالخبرات:\n'+e+'\n\nالمهارات:\n- العمل تحت الضغط\n- اللغة الإنجليزية\n- الحاسب الآلي\n\nالهدف: الحصول على وظيفة في مجال '+s+' والمساهمة في رؤية 2030';
 ...
 function generateQR(){
  ...
  if(!val){alert('أدخل رابط أو نص');return;}
  qrImage.src='https://api.qrserver.com/v1/create-qr-code/?size=300x300&data='+encodeURIComponent(val);
  qrResult.style.display='block';
 }

 function downloadQR(){
  if(!qrImage.src){alert('ولّد QR أولاً');return;}
 ...
 function genIdea(){
  ...
  const ideas={'5000':['متجر إلكتروني منتجات محلية (عسل، سمن)','خدمة كتابة بحوث للطلاب','تصميم سير ذاتية'],'10000':['مغسلة ملابس متنقلة','عربة فود ترك قهوة مختصة','متجر تغليف هدايا'],'20000':['مشروع دروس خصوصية أونلاين','استوديو تصوير صغير','محل اكسسوارات جوالات'],'50000':['مقهى طلابي قرب الجامعة','شركة توصيل داخلي','مركز تدريب حاسب']};
  let list=ideas[b];
  let rnd=list[Math.floor(Math.random()*list.length)];
  ideaRes.style.display='block';
  ideaRes.innerHTML='💡 فكرة مقترحة برأس مال '+b+' ريال:<br><b style="color:#facc15;font-size:18px">'+rnd+'</b><br><br>اسأل نبراس: "سوي لي دراسة جدوى لـ '+rnd+'"';
 }

 addBtn.addEventListener('click',addSubject);
 calcBtn.addEventListener('click',calcGPA);
 cvBtn.addEventListener('click',makeCV);
 qrBtn.addEventListener('click',generateQR);
 dloadBtn.addEventListener('click',downloadQR);
 ideaBtn.addEventListener('click',genIdea);
 attachRemoveEvents();
});
</script></body></html>"""
