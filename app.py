...
# ========== NEW: Voice Session Endpoint ==========
@app.route('/voice-session', methods=['POST'])
@limiter.limit("2 per minute")
def voice_session():
    try:
        # إنشاء جلسة صوتية باستخدام OpenAI Realtime API
        session_data = client.realtime.sessions.create(
            model="gpt-4o-realtime-preview-2024-12-17",
            voice="verse"  # اختيار الصوت: verse, alloy, echo, shimmer
        )
        return jsonify({
...
async function startVoiceCall() {
    try {
        ...
        // 1. Request session from Flask backend
        const sessionResponse = await fetch('/voice-session', { method: 'POST' });
        const sessionData = await sessionResponse.json();
        if (!sessionResponse.ok) {
            alert('❌ فشل إنشاء جلسة الصوت: ' + (sessionData.error || ''));
        ...
        // 3. Receive audio from assistant
        pc.ontrack = function(event) {
            assistantAudio.srcObject = event.streams[0];
            assistantAudio.play().catch(e => console.error('Audio play error:', e));
        };
        ...
        // 5. Data channel for events (optional)
        dataChannel = pc.createDataChannel('oai-events');
        dataChannel.onopen = () => console.log('Data channel opened');
        dataChannel.onmessage = (event) => console.log('Realtime event:', event.data);
        ...
        if (!sdpResponse.ok) {
            ...
            alert('❌ فشل الاتصال الصوتي: ' + errorText);
        ...
        addMessage('📞 المكالمة الصوتية نشطة، تحدث الآن.', 'bot', true);

    } catch (error) {
        console.error(error);
        alert('❌ حدث خطأ في المكالمة الصوتية: ' + error.message);
        endVoiceCall();
        voiceBtn.disabled = false;
...
function endVoiceCall() {
    ...
    voiceBtn.disabled = false;
    voiceBtn.classList.remove('active');
    addMessage('📴 تم إنهاء المكالمة الصوتية.', 'bot', true);
}
// =======================================
