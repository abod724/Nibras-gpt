...
def get_db():
    ...
    return conn
def init_db():
 conn=get_db();conn.execute('''CREATE TABLE IF NOT EXISTS conversations (user_id TEXT, conv_id TEXT PRIMARY KEY, messages TEXT, timestamp TEXT, title TEXT)''')
...
def chat():
 try:
  ...
  # تحميل التاريخ من السيرفر بدل المتصفح لأمان أكثر
