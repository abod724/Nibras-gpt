...
# ========== نظام التعليمات الجديد (خالي من التعارض) ==========
SP=f"""
...
**⚠️ قاعدة التنسيق الوحيدة (يجب تطبيقها في كل رد):**
- اكتب ردك في **فقرات نصية متصلة** من اليمين لليسار.
- استخدم `**` لتظليل الكلمات المهمة (الخط العريض).
- يمكنك استخدام `#` للعناوين إذا لزم الأمر.
- **إذا أردت سرد خطوات أو نقاط، اكتبها في جملة واحدة طويلة** تفصل بين الأرقام بفواصل أو مسافات.
  - ✅ **مثال صحيح:** "١. حمص البن، ٢. اطحنه، ٣. اغلي الماء، ٤. صبّه في الفنجان."
  - ❌ **ممنوع** كتابة الأرقام أو النقاط في سطور منفصلة (مثل: 
    ١. حمص البن
    ٢. اطحنه )
- اترك سطراً فارغاً بين الفقرات فقط.
  ...
  if use_web:
   try:
    ...
    sr=client.responses.create(
        model=model,
        instructions=f"{SP}\n\nسياق المحادثة السابقة:\n{fc}\n\n**هام:** اكتب الرد بفقرات متصلة ولا تستخدم القوائم العمودية.",
