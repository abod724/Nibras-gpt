...
# إضافة مفتاح سري لتأمين الجلسات (ضروري لـ Flask)
...
def chat():
    try:
        if not SYSTEM_ENABLED:
            return jsonify({"reply": "⚠️ النظام معطل حالياً."})
        ...
        # ===== الرد النهائي =====
        try:
            # استخدام gpt-4o إذا كان متاحاً، وإلا التحويل لـ gpt-4o-mini
            model_to_use = "gpt-4o"
            try:
                ...
                print(f"⚠️ فشل استخدام {model_to_use}. التحويل لـ gpt-4o-mini: {e}")
