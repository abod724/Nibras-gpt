...
import openai, os, secrets, json, hashlib, base64, re, sqlite3, requests
...
# ===== دالة الصوت باستخدام gTTS (متزامنة تماماً) =====
def generate_speech(text, gender):
    try:
        ...
        print(f"❌ فشل الصوت (gTTS): {e}")
        return None

# =============================================
# القوالب (HTML) - كاملة كما في مشروعك السابق
# =============================================
...
# =============================================
# دوال المساعدة (نفس الكود القديم)
# =============================================
def get_user_id():
    ...
    if request.method == 'POST':
        ...
        if email == admin_email:
            if not admin_password:
                return render_template_string(LH, error="خطأ: لم يتم إعداد كلمة مرور الأدمن في الخادم.")
...
def shared_conversation(cid):
    conn = get_db()
    row = conn.execute("SELECT messages, title FROM conversations WHERE conv_id=?", (cid,)).fetchone()
    conn.close()
    if row:
        messages = json.loads(row[0])
        title = row[1] or "محادثة نبراس"
        return render_template_string(SPH, messages=messages, title=title)
    ...
    try:
        ...
        if not is_admin:
            if not check_guest_limit_safe(uid):
                ...
                if conv_id is None:
                    sm[uid] = []
                sm[uid].append({"role": "user", "content": user_message})
                sm[uid].append({"role": "assistant", "content": limit_reply})
                nid = save_user_conversation(uid, sm[uid], conv_id)
                return jsonify({"reply": limit_reply, "conv_id": nid, "audio": None})
        ...
        if is_image_request(user_message) and not has_image:
            print(f"🎨 طلب رسم/فيديو مجاني من {uid}")
            video_keywords = ["فيديو", "ابي فيديو", "عرض فيديو"]
            is_video = any(kw in user_message for kw in video_keywords)

            if is_video:
                video_result = search_video(user_message)
                if video_result and video_result.startswith("ERROR:"):
                    ...
                    nid = save_user_conversation(uid, sm[uid], conv_id)
                    return jsonify({"reply": reply, "conv_id": nid, "image_url": None})
                elif video_result:
            ...
            img_result = generate_image(user_message)
        ...
        if has_image and not is_admin:
            ...
            sm[uid].append({"role": "assistant", "content": reply})
            nid = save_user_conversation(uid, sm[uid], conv_id)
            return jsonify({"reply": reply, "conv_id": nid, "audio": None})
        ...
        server_hist = load_conversation_by_id(uid, conv_id) if conv_id else []
        if not server_hist:
            server_hist = sm.get(uid, [])
        server_hist.append({"role": "user", "content": user_message})
        sm[uid] = server_hist
        ch = server_hist[-30:]
        ...
        if image_data and is_admin:
            msgs.append({"role": "user", "content": [{"type": "text", "text": user_message or "حلل هذه الصورة"}, {"type": "image_url", "image_url": {"url": image_data}}]})

        # ===== البحث بالويب (للأدمن فقط) =====
        if use_web:
            try:
                fc = ""
                for m in msgs:
