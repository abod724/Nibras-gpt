...
# ============ نظام الأكواد المؤمنة (HMAC) ============
def generate_access_code(days):
    expiry=datetime.now()+timedelta(days=int(days))
    date_str=expiry.strftime("%Y%m%d")
    signature=hmac.new(CODE_SECRET.encode(),date_str.encode(),hashlib.sha256).hexdigest()[:10]
    return f"{date_str}-{signature}",expiry.strftime("%Y-%m-%d")
...
def verify_access_code(code):
    try:
        ...
        expected=hmac.new(CODE_SECRET.encode(),date_str.encode(),hashlib.sha256).hexdigest()[:10]
...
# ============ تسجيل الدخول (إيميل + كلمة مرور + كود موقّع) ============
...
def login():
    if request.method=='POST':
        ...
        if user and user['password_hash']:
            ...
            # تحقق من انتهاء الكود لو موجود
            if user['code_expiry']:
                try:
                    exp=datetime.fromisoformat(user['code_expiry'])
                    if exp.date()<datetime.now().date():
                        return render_template_string(LH,error="انتهى صلاحية كودك. تواصل مع المطور.")
                except:pass
            session.clear();session.permanent=True
            session['user_email']=e;session['is_admin']=False
            return redirect(url_for('index'))
        
        # 3. مستخدم جديد: يحتاج كود
        if not code:
            return render_template_string(LH,error="للتسجيل الجديد، أدخل كود الوصول.")
        
        expiry=verify_access_code(code)
        if not expiry:
            return render_template_string(LH,error="❌ الكود غير صحيح أو منتهي الصلاحية.")
...
def admin_dashboard():
    ...
    invite_form="""<div class="card"><h3>🔐 توليد كود وصول</h3><p style="font-size:14px;color:#8b949e;margin-bottom:15px;">اختر المدة ثم اضغط "توليد". الكود يحمل توقيعاً سرياً يمنع التزوير.</p><a href="/admin/generate" style="display:block;background:#58a6ff;color:#fff;text-decoration:none;padding:12px;border-radius:8px;text-align:center;font-weight:bold;">اذهب لصفحة التوليد</a></div>"""
...
def admin_generate():
    if not session.get('is_admin'):return redirect(url_for('admin_login'))
    return """<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>توليد كود - نبراس</title><style>body{font-family:'Segoe UI',Tahoma;background:#0d1117;color:#c9d1d9;padding:20px;display:flex;justify-content:center;align-items:center;min-height:100dvh;margin:0}.box{background:#161b22;padding:30px;border-radius:20px;border:1px solid #30363d;max-width:500px;width:100%;text-align:center}h2{color:#58a6ff;margin-bottom:20px}input{width:100%;padding:14px;margin:10px 0;border-radius:10px;border:1px solid #30363d;background:#0d1117;color:#c9d1d9;font-size:16px;box-sizing:border-box;text-align:center}button{background:#58a6ff;color:#fff;border:none;padding:14px 30px;border-radius:10px;cursor:pointer;font-weight:bold;font-size:16px;width:100%;margin-top:10px}button:hover{background:#79c0ff}.code-box{background:#0d1117;padding:20px;border-radius:10px;margin-top:20px;border:2px dashed #58a6ff;font-size:20px;color:#58a6ff;word-break:break-all;font-family:monospace;letter-spacing:2px;text-align:center}.info{font-size:13px;color:#8b949e;margin-top:15px}.back{display:inline-block;margin-top:20px;color:#58a6ff;text-decoration:none}</style></head><body><div class="box"><h2>🔐 توليد كود وصول</h2><form onsubmit="event.preventDefault();const d=document.getElementById('days').value;fetch('/admin/generate_api?days='+d).then(r=>r.json()).then(d=>{if(d.code){document.getElementById('result').innerHTML='<div class="code-box">'+d.code+'</div><div class="info">ينتهي في: <b>'+d.expiry+'</b><br>انسخه وأرسله للمستخدم.</div>';}else{alert('خطأ: '+(d.error||'فشل'));}})"><input type="number" id="days" placeholder="عدد الأيام" value="30" min="1" max="365" required><button type="submit">توليد الكود</button></form><div id="result"></div><a href="/admin" class="back">⬅ العودة للوحة التحكم</a></div></body></html>"""
...
def admin_generate_api():
    ...
    try:
        ...
        code,expiry=generate_access_code(days)
        return jsonify({"code":code,"expiry":expiry})
...
def chat():
    try:
        ...
        # لو مستخدم: تحقق من انتهاء الكود
        if has_user:
