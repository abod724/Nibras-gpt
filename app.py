...
# =============================================
# 1. المتغيرات الضخمة (منسوخة من كودك القديم)
# =============================================
...
# =============================================
# 3. دوال قاعدة البيانات (نفس الكود القديم)
# =============================================
...
# =============================================
# 4. دوال الصور والفيديو والصوت (نفس الكود)
# =============================================
...
# =============================================
# 5. المعرفة الأساسية (ملف المعرفة)
# =============================================
...
# مجلدات ثابتة (اختيارية)
if os.path.exists("static"):
    app.mount("/static", StaticFiles(directory="static"), name="static")
if os.path.exists(".well-known"):
    app.mount("/.well-known", StaticFiles(directory=".well-known"), name="well-known")

# دوال الجلسات (بديل session)
def get_user_id_from_request(request: Request) -> str:
    cookie = request.cookies.get("nibras_session")
    if cookie:
        try:
...
def set_user_cookie(response, user_id):
    ...
    response.set_cookie(key="nibras_session", value=token, httponly=True, max_age=3600*24*30)

# دوال عرض القوالب (من النصوص)
from jinja2 import Environment, BaseLoader

def render_html(template_string, **kwargs):
    env = Environment(loader=BaseLoader())
    template = env.from_string(template_string)
    return HTMLResponse(content=template.render(**kwargs))

# =============================================
# 7. نقاط النهاية (Endpoints) – مطابقة لكود Flask
# =============================================
...
async def index(request: Request):
    ...
    return render_html(HT, session=fake_session, request=request)

@app.get("/tools", response_class=HTMLResponse)
async def tools_page():
    return render_html(TOOLS_HTML)

@app.get("/login", response_class=HTMLResponse)
async def login_get(request: Request, error: str = None):
    return render_html(LH, request=request, error=error)
...
async def login_post(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    if email == admin_email:
        if not admin_password:
            return render_html(LH, request=request, error="خطأ: لم يتم إعداد كلمة مرور الأدمن في الخادم.")
        if secrets.compare_digest(password, admin_password):
            response = RedirectResponse(url="/", status_code=302)
            set_user_cookie(response, "admin_" + admin_email)
            return response
        else:
            return render_html(LH, request=request, error="كلمة مرور الأدمن غير صحيحة.")
    elif "@" in email:
        response = RedirectResponse(url="/", status_code=302)
        set_user_cookie(response, "user_" + email)
        return response
    else:
        return render_html(LH, request=request, error="يرجى إدخال بريد إلكتروني صحيح.")
...
async def load_conversation(request: Request, cid: str):
    user_id = get_user_id_from_request(request)
    msgs = load_conversation_by_id(user_id, cid)
    if msgs is None:
...
async def delete_message(request: Request):
    ...
    if not cid or idx is None:
        return JSONResponse({"status": "error", "message": "بيانات ناقصة"}, status_code=400)
    msgs = load_conversation_by_id(user_id, cid)
    if not msgs:
    ...
    del msgs[idx]
    save_user_conversation(user_id, msgs, cid)
    return {"status": "ok"}
...
async def shared_conversation(request: Request, cid: str):
    conn = get_db()
    r = conn.execute("SELECT messages, title FROM conversations WHERE conv_id=?", (cid,)).fetchone()
    conn.close()
    if r:
        messages = json.loads(r[0])
        title = r[1] or "محادثة نبراس"
        return render_html(SPH, messages=messages, title=title)
...
async def set_gender(request: Request):
    ...
    response.set_cookie("voice_gender", gender, max_age=3600*24*30)
    return response

# =============================================
# 8. نقطة النهاية الأساسية: /chat (المحادثة)
# =============================================
...
async def chat_endpoint(request: Request):
    try:
        ...
        if not um:
            return JSONResponse({"reply": "اكتب شيء أساعدك فيه"})

        # حد الزوار
        if not is_admin:
            if not check_guest_limit_safe(user_id):
                ...
                if cid is None:
                    sm[user_id] = []
                sm[user_id].append({"role": "user", "content": um})
                sm[user_id].append({"role": "assistant", "content": reply_limit})
                nid = save_user_conversation(user_id, sm[user_id], cid)
                return JSONResponse({"reply": reply_limit, "conv_id": nid, "audio": None})
            ...
            if cached:
                if cid is None:
                    sm[user_id] = []
                sm[user_id].append({"role": "user", "content": um})
                sm[user_id].append({"role": "assistant", "content": cached})
                nid = save_user_conversation(user_id, sm[user_id], cid)
                return JSONResponse({"reply": cached + "\n\n⚡ جواب سريع من الذاكرة", "conv_id": nid, "audio": None})

        # توليد الصور والفيديو المجاني
        ...
        if is_image_request(um) and not has_image:
            print(f"🎨 طلب رسم/فيديو مجاني من {user_id}")
            ...
            if is_video:
                ...
                if video_result and video_result.startswith("ERROR:"):
                    ...
                    sm[user_id].append({"role": "user", "content": um})
                    sm[user_id].append({"role": "assistant", "content": reply})
                    nid = save_user_conversation(user_id, sm[user_id], cid)
                    return JSONResponse({"reply": reply, "conv_id": nid})
                elif video_result:
                    reply = f"🎬 إليك الفيديو الذي طلبتـه:"
                    reply_with_url = reply + "\n" + video_result
                    sm[user_id].append({"role": "user", "content": um})
                    sm[user_id].append({"role": "assistant", "content": reply_with_url})
                    nid = save_user_conversation(user_id, sm[user_id], cid)
                    return JSONResponse({"reply": reply_with_url, "image_url": video_result, "conv_id": nid})
                else:
            ...
            if img_result and img_result.startswith("ERROR:"):
                ...
                sm[user_id].append({"role": "assistant", "content": reply_with_url})
                nid = save_user_conversation(user_id, sm[user_id], cid)
                return JSONResponse({"reply": reply_with_url, "image_url": img_result, "conv_id": nid})
            else:
        ...
        server_hist = load_conversation_by_id(user_id, cid) if cid else []
        if not server_hist:
            server_hist = sm.get(user_id, [])
        server_hist.append({"role": "user", "content": um})
        sm[user_id] = server_hist
        ch = server_hist[-30:]
        ...
        if image_data and is_admin:
            msgs.append({"role": "user", "content": [{"type": "text", "text": um or "حلل هذه الصورة"}, {"type": "image_url", "image_url": {"url": image_data}}]})
        ...
        return JSONResponse({"reply": reply, "audio": audio, "conv_id": nid})
