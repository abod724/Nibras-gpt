...
# ====== دالة توليد الصورة باستخدام Unsplash (مع تنظيف النص) ======
def generate_image(prompt):
    try:
        ...
        query = requests.utils.quote(cleaned)
...
def chat():
 try:
  ...
  if allow_img and is_image_request(um):
   ...
   # ====== الشرط الجديد: فقط إذا كان النص فارغاً ======
   if not cleaned:
  ...
  # ====== إعادة تاريخ المحادثة إلى 10 رسائل ======
  ch = sm[uid][-10:]
  # اقتطاع الرسائل الطويلة (أكثر من 500 حرف)
  ...
  try:
   ...
   # ====== رسالة ودية عند تجاوز حد الرموز ======
