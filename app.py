...
def save_user_conversation(user_id, conversation, conv_id=None):
    ...
    c = conn.cursor()
    if conv_id is None:
        # إنشاء عنوان للمحادثة من أول رسالة
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Arial, sans-serif; }
        html, body { margin: 0; padding: 0; width: 100%; height: 100%; overflow: hidden; background: var(--bg-body); transition: background 0.3s ease; }
        body { display: flex; justify-content: center; align-items: center; position: relative; }
        .app { position: fixed; top: 0; left: 0; right: 0; bottom: 0; width: 100%; max-width: 450px; margin: 0 auto; background: var(--bg-app); display: flex; flex-direction: column; overflow: hidden; transition: background 0.3s ease; box-shadow: 0 0 20px var(--shadow-color); }
        @media (min-width: 600px) {
            .app { top: 50%; left: 50%; transform: translate(-50%, -50%); bottom: auto; right: auto; height: 100dvh; max-height: 100dvh; border-radius: 20px; }
        }
        @media (orientation: landscape) and (max-width: 599px) {
            .app { max-width: 100%; border-radius: 0; box-shadow: none; top: 0; left: 0; right: 0; bottom: 0; transform: none; height: 100%; max-height: 100%; }
        }
            ...
            if (sender === 'bot' && !isSystem && !generatedImageUrl) {
                ...
                function typeChar() {
                    if (index < displayText.length) {
                        ...
                        index++;
                        if (!userInteracted) chatBox.scrollTop = chatBox.scrollHeight;
...
def chat():
    try:
        ...
        # تحديد النموذج والميزات المتاحة
        if is_admin:
            model = "gpt-4o"
            use_web_search = True
            allow_images = True
            ...
            use_web_search = False
            allow_images = False
            # بعد انتهاء التجربة، لا نضيف أي رسالة تنبيه في الردود
        ...
        # بناء رسائل النظام والمحادثة
        messages = [{"role": "system", "content": SYSTEM_PROMPT}]
        for entry in chat_history:
            messages.append({"role": entry["role"], "content": entry["content"]})

        # إضافة الصورة المرفقة إن وجدت
        ...
        # البحث بالويب (للمستخدمين المميزين)
        if use_web_search:
            try:
                full_context = ""
                for msg in messages:
        ...
        # تحديث عدد المحاولات التجريبية (بدون إضافة أي رسالة تنبيه)
        if is_trial_user and trial_remaining > 0:
            session['trial_remaining'] = trial_remaining - 1
            if session['trial_remaining'] == 0:
                session['is_trial_expired'] = True
