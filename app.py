...
import openai,os,secrets,json,hashlib,asyncio,base64,re,sqlite3,requests,edge_tts,hmac
...
CODE_SECRET=os.environ.get("CODE_SECRET","nibras-secret-change-me-please-2026")
VAPID_PUBLIC_KEY=os.environ.get("VAPID_PUBLIC_KEY","")
VAPID_PRIVATE_KEY=os.environ.get("VAPID_PRIVATE_KEY","")
VAPID_EMAIL=os.environ.get("VAPID_EMAIL","mailto:nibrasalsd@gmail.com")
...
def init_db():
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS users (email TEXT PRIMARY KEY,role TEXT DEFAULT 'user',voice_gender TEXT DEFAULT 'male',password_hash TEXT,code_expiry TEXT,created_at TEXT)''')
    conn.execute('''CREATE TABLE IF NOT EXISTS subscriptions (id INTEGER PRIMARY KEY AUTOINCREMENT,email TEXT,endpoint TEXT UNIQUE,data TEXT,created_at TEXT)''')
    try:conn.execute("ALTER TABLE users ADD COLUMN code_expiry TEXT")
    except:pass
...
def generate_access_code(email,days):
    ...
    data=f"{date_str}-{email_hash}"
    signature=hmac.new(CODE_SECRET.encode(),data.encode(),hashlib.sha256).hexdigest()[:8]
...
def verify_access_code(code,email):
    try:
        ...
        date_str,email_hash,signature=parts
        data=f"{date_str}-{email_hash}"
        expected=hmac.new(CODE_SECRET.encode(),data.encode(),hashlib.sha256).hexdigest()[:8]
        if not hmac.compare_digest(signature,expected):return None
        entered_hash=hashlib.sha256(email.lower().strip().encode()).hexdigest()[:8]
        if not hmac.compare_digest(email_hash,entered_hash):return None
        expiry=datetime.strptime(date_str,"%Y%m%d")
        if expiry.date()<datetime.now().date():return None
        return expiry
    except:return None
...
def get_user_role(email):
    ...
    conn=get_db();row=conn.execute("SELECT role FROM users WHERE email=?",(email,)).fetchone();conn.close()
...
def send_push_notification(subscription_data, title, body):
    if not WEBPUSH_AVAILABLE or not VAPID_PRIVATE_KEY:return False
    try:
        webpush(subscription_info=subscription_data,data=json.dumps({"title":title,"body":body}),vapid_private_key=VAPID_PRIVATE_KEY,vapid_claims={"sub":VAPID_EMAIL})
        return True
    except Exception as e:print(f"❌ إشعار: {e}");return False

def send_to_all(title, body):
    conn=get_db();rows=conn.execute("SELECT id, endpoint, data FROM subscriptions").fetchall();conn.close()
    success=0;failed_ids=[]
    for row in rows:
        try:
            sub_data=json.loads(row['data'])
            if send_push_notification(sub_data,title,body):success+=1
            else:failed_ids.append(row['id'])
        except:failed_ids.append(row['id'])
    if failed_ids:
        conn=get_db()
        for fid in failed_ids:conn.execute("DELETE FROM subscriptions WHERE id=?",(fid,))
        conn.commit();conn.close()
    return success
...
async function toggleNotifications(){
    ...
    try{
        ...
        if(sub){
            ...
            nb.classList.remove('active');nb.title='تفعيل الإشعارات';
            showToast('🔕 تم إلغاء الإشعارات');
        }else{
            const perm=await Notification.requestPermission();
            if(perm!=='granted'){showToast('❌ تم رفض الإذن');return;}
            const newSub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:urlBase64ToUint8Array(window.VAPID_PUBLIC_KEY)});
            await fetch('/subscribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({subscription:newSub})});
            nb.classList.add('active');nb.title='الإشعارات مفعّلة';
            showToast('🔔 تم تفعيل الإشعارات');
        }
    }catch(e){console.error(e);showToast('❌ خطأ: '+e.message);}
}

if(nb){nb.addEventListener('click',toggleNotifications);initNotifications();}
...
window.deleteMyData=function(){
    if(!confirm('⚠️ سيتم حذف جميع محادثاتك فقط. حسابك سيبقى شغالاً.\n\nهل أنت متأكد؟'))return;
    fetch('/delete_my_data',{method:'POST',headers:{'Content-Type':'application/json'}})
        .then(r=>r.json()).then(d=>{
            if(d.status==='success'){alert('✅ تم حذف جميع محادثاتك.');window.location.href='/'}
...
window.deleteMyAccount=function(){
    if(!confirm('⚠️ تحذير: سيتم حذف حسابك بالكامل (الإيميل + كلمة المرور + الكود + كل المحادثات).\n\nهل أنت متأكد؟'))return;
...
@app.route('/')
def index():return render_template_string(HT,vapid_key=VAPID_PUBLIC_KEY)
...
def subscribe():
    try:
        d=request.get_json();sub=d.get('subscription')
        if not sub:return jsonify({"status":"error","message":"بيانات ناقصة"}),400
        email=session.get('user_email') or ('admin' if session.get('is_admin') else 'guest')
        endpoint=sub.get('endpoint','')
        conn=get_db()
        conn.execute("INSERT OR REPLACE INTO subscriptions (email, endpoint, data, created_at) VALUES (?,?,?,?)",(email,endpoint,json.dumps(sub),datetime.now().isoformat()))
        conn.commit();conn.close()
...
def unsubscribe():
    try:
        d=request.get_json();endpoint=d.get('endpoint','')
        conn=get_db();conn.execute("DELETE FROM subscriptions WHERE endpoint=?",(endpoint,));conn.commit();conn.close()
        return jsonify({"status":"ok"})
    except Exception as e:return jsonify({"status":"error","message":str(e)}),500
...
def login():
    if request.method=='POST':
        ...
        if not e or "@" not in e:return render_template_string(LH,error="يرجى إدخال بريد صحيح.")
        ...
        if user and user['password_hash']:
            ...
            if user['code_expiry']:
                try:
                    exp=datetime.fromisoformat(user['code_expiry'])
                    if exp.date()<datetime.now().date():expired=True
                except:pass
            if expired:
                if not code:
                    conn.close()
                    return render_template_string(LH,error="انتهت صلاحية كودك. أدخل كود جديد من المطور.")
                new_expiry=verify_access_code(code,e)
                if not new_expiry:
                    conn.close()
                    return render_template_string(LH,error="❌ الكود الجديد غير صحيح، منتهي، أو مخصص لإيميل آخر.")
                conn.execute("UPDATE users SET code_expiry=? WHERE email=?",(new_expiry.isoformat(),e))
                conn.commit()
            conn.close()
            session.clear();session.permanent=True
        ...
        expiry=verify_access_code(code,e)
        if not expiry:
            conn.close()
            return render_template_string(LH,error="❌ الكود غير صحيح، منتهي، أو مخصص لإيميل آخر.")
        
        conn.execute("""INSERT INTO users (email,role,voice_gender,password_hash,code_expiry,created_at)
                        VALUES (?,?,?,?,?,?)
                        ON CONFLICT(email) DO UPDATE SET
                        password_hash=excluded.password_hash,
                        code_expiry=excluded.code_expiry""",
                     (e,'user','male',generate_password_hash(p),expiry.isoformat(),datetime.now().isoformat()))
        conn.commit();conn.close()
...
def delete_message():
    try:
        ...
        if not msgs:return jsonify({"status":"error","message":"غير موجودة"}),404
        if idx<0 or idx>=len(msgs):return jsonify({"status":"error","message":"غير موجودة"}),404
        del msgs[idx];save_user_conversation(uid,msgs,cid);return jsonify({"status":"ok"})
    except Exception as e:return jsonify({"status":"error","message":str(e)}),500

@app.route('/delete_my_data',methods=['POST'])
def delete_my_data():
    uid=get_user_id();conn=get_db();conn.execute("DELETE FROM conversations WHERE user_id=?",(uid,));conn.commit();conn.close()
    return jsonify({"status":"success","message":"تم حذف محادثاتك"})
...
def delete_my_account():
    ...
    if email:
        ...
        conn.execute("DELETE FROM users WHERE email=? AND role!='admin'",(email,))
    conn.commit();conn.close()
    session.clear()
...
def admin_dashboard():
    ...
    users_list=conn.execute("SELECT email,code_expiry FROM users WHERE role='user' ORDER BY created_at DESC LIMIT 20").fetchall()
    conn.close()
    recent_html=""
    for row in recent:
        user=row[0][:15]+"..." if len(row[0])>15 else row[0];title=row[1] or "بدون عنوان";time=row[2][:16] if row[2] else ""
    ...
    users_html=""
    for u in users_list:
        exp=u[1][:10] if u[1] else "—"
        status="✅ نشط" if u[1] and datetime.fromisoformat(u[1]).date()>=datetime.now().date() else "⛔ منتهي"
        users_html+=f'<div class="conv-item"><b>{u[0]}</b><small>ينتهي: {exp} | {status}</small></div>'
    if not users_html:users_html="<p style='color:#8b949e;text-align:center;'>لا يوجد مستخدمون</p>"
    return f"""<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>لوحة تحكم نبراس</title><style>body{{font-family:'Segoe UI',Tahoma;background:#0d1117;color:#c9d1d9;padding:20px;margin:0}}.container{{max-width:600px;margin:auto}}h1{{color:#58a6ff;text-align:center}}.card{{background:#161b22;border-radius:15px;padding:15px;margin:15px 0;border:1px solid #30363d}}.stat{{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #21262d}}.stat:last-child{{border:none}}.num{{color:#58a6ff;font-weight:bold;font-size:18px}}.conv-item{{padding:10px 0;border-bottom:1px solid #21262d}}.conv-item small{{color:#8b949e;display:block;font-size:12px}}.link-btn{{display:block;background:#58a6ff;color:#fff;text-decoration:none;padding:12px;border-radius:8px;text-align:center;font-weight:bold;margin-top:10px}}.link-btn.green{{background:#22c55e}}.back{{display:block;text-align:center;color:#58a6ff;text-decoration:none;margin-top:20px}}</style></head><body><div class="container"><h1>📊 لوحة تحكم نبراس</h1><div class="card"><div class="stat"><span>👥 المستخدمون</span><span class="num">{users_count}</span></div><div class="stat"><span>💬 المحادثات</span><span class="num">{total_convs}</span></div><div class="stat"><span>📅 اليوم</span><span class="num">{today_convs}</span></div><div class="stat"><span>🔔 المشتركون بالإشعارات</span><span class="num">{subs_count}</span></div></div><div class="card"><h3>🔐 توليد كود وصول</h3><p style="font-size:14px;color:#8b949e;margin-bottom:15px;">أدخل بريد المستخدم واختر المدة.</p><a href="/admin/generate" class="link-btn">اذهب لصفحة التوليد</a></div><div class="card"><h3>📢 إرسال إشعار للجميع</h3><p style="font-size:14px;color:#8b949e;margin-bottom:15px;">أرسل رسالة لجميع المشتركين.</p><a href="/admin/notify" class="link-btn green">اذهب لصفحة الإشعارات</a></div><div class="card"><h3>👥 المستخدمون المسجلون</h3>{users_html}</div><div class="card"><h3>🕒 آخر 10 محادثات</h3>{recent_html}</div><a href="/" class="back">⬅ الرئيسية</a></div></body></html>"""

@app.route('/admin/generate')
def admin_generate():
    if not session.get('is_admin'):return redirect(url_for('admin_login'))
    return """<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>توليد كود - نبراس</title><style>body{font-family:'Segoe UI',Tahoma;background:#0d1117;color:#c9d1d9;padding:20px;display:flex;justify-content:center;align-items:center;min-height:100dvh;margin:0}.box{background:#161b22;padding:30px;border-radius:20px;border:1px solid #30363d;max-width:500px;width:100%;text-align:center}h2{color:#58a6ff;margin-bottom:20px}input{width:100%;padding:14px;margin:10px 0;border-radius:10px;border:1px solid #30363d;background:#0d1117;color:#c9d1d9;font-size:16px;box-sizing:border-box;text-align:center}button{background:#58a6ff;color:#fff;border:none;padding:14px 30px;border-radius:10px;cursor:pointer;font-weight:bold;font-size:16px;width:100%;margin-top:10px}button:hover{background:#79c0ff}.code-box{background:#0d1117;padding:20px;border-radius:10px;margin-top:20px;border:2px dashed #58a6ff;font-size:18px;color:#58a6ff;word-break:break-all;font-family:monospace;letter-spacing:2px;text-align:center}.info{font-size:13px;color:#8b949e;margin-top:15px}.warning{font-size:13px;color:#f0ad4e;margin-top:10px;background:#2d1f0a;padding:10px;border-radius:8px;border:1px solid #5c3d10}.back{display:inline-block;margin-top:20px;color:#58a6ff;text-decoration:none}</style></head><body><div class="box"><h2>🔐 توليد كود وصول</h2><form id="genForm"><input type="email" id="email" placeholder="بريد المستخدم (إلزامي)" required><input type="number" id="days" placeholder="عدد الأيام" value="30" min="1" max="365" required><button type="submit">توليد الكود</button></form><div id="result"></div><div class="warning">⚠️ الكود سيعمل فقط مع هذا الإيميل بالتحديد.</div><a href="/admin" class="back">⬅ العودة للوحة التحكم</a></div>
<script>
document.getElementById('genForm').addEventListener('submit',function(e){
    e.preventDefault();
    var email=document.getElementById('email').value.trim();
    var days=document.getElementById('days').value;
    if(!email||email.indexOf('@')<0){alert('أدخل بريداً صحيحاً');return;}
    fetch('/admin/generate_api?email='+encodeURIComponent(email)+'&days='+days)
        .then(function(r){return r.json();})
        .then(function(data){
            if(data.code){document.getElementById('result').innerHTML='<div class="code-box">'+data.code+'</div><div class="info">للبريد: <b>'+data.email+'</b><br>ينتهي في: <b>'+data.expiry+'</b></div>';}
            else{alert('خطأ: '+(data.error||'فشل'));}
        }).catch(function(){alert('حدث خطأ في الاتصال');});
});
</script></body></html>"""

@app.route('/admin/generate_api')
def admin_generate_api():
    if not session.get('is_admin'):return jsonify({"error":"غير مصرح"}),403
    try:
        email=request.args.get('email','').strip().lower()
        days=int(request.args.get('days',30))
        if not email or "@" not in email:return jsonify({"error":"بريد غير صحيح"}),400
        if days<1 or days>365:return jsonify({"error":"المدة 1-365 يوم"}),400
        code,expiry=generate_access_code(email,days)
...
def admin_notify():
    if not session.get('is_admin'):return redirect(url_for('admin_login'))
    conn=get_db();subs_count=conn.execute("SELECT COUNT(*) FROM subscriptions").fetchone()[0];conn.close()
    return f"""<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>إرسال إشعار - نبراس</title><style>body{{font-family:'Segoe UI',Tahoma;background:#0d1117;color:#c9d1d9;padding:20px;display:flex;justify-content:center;align-items:center;min-height:100dvh;margin:0}}.box{{background:#161b22;padding:30px;border-radius:20px;border:1px solid #30363d;max-width:500px;width:100%}}h2{{color:#22c55e;margin-bottom:20px;text-align:center}}p.info{{font-size:14px;color:#8b949e;text-align:center;margin-bottom:20px}}label{{display:block;font-size:14px;color:#c9d1d9;margin:15px 0 5px}}input,textarea{{width:100%;padding:14px;border-radius:10px;border:1px solid #30363d;background:#0d1117;color:#c9d1d9;font-size:15px;box-sizing:border-box;font-family:inherit}}textarea{{min-height:100px;resize:vertical}}button{{background:#22c55e;color:#fff;border:none;padding:14px 30px;border-radius:10px;cursor:pointer;font-weight:bold;font-size:16px;width:100%;margin-top:20px}}button:hover{{background:#16a34a}}button:disabled{{background:#444;cursor:not-allowed}}.result{{margin-top:20px;padding:15px;border-radius:10px;background:#0d1117;border:1px solid #30363d;font-size:14px;text-align:center}}.back{{display:block;text-align:center;color:#58a6ff;text-decoration:none;margin-top:20px}}.quick{{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}}.quick button{{background:#30363d;color:#c9d1d9;padding:8px 14px;font-size:13px;width:auto;margin:0;border-radius:20px}}.quick button:hover{{background:#58a6ff;color:#fff}}</style></head><body><div class="box"><h2>📢 إرسال إشعار</h2><p class="info">سيُرسل لجميع المشتركين ({subs_count} جهاز)</p><label>العنوان</label><input type="text" id="title" placeholder="نبراس" value="نبراس"><label>الرسالة</label><textarea id="body" placeholder="اكتب رسالتك..."></textarea><label>رسائل سريعة:</label><div class="quick"><button onclick="setMsg('ها ما ودك نسولف اليوم؟ 💬')">ما ودك نسولف؟</button><button onclick="setMsg('وين اليوم مختفي؟ 👀')">وين مختفي؟</button><button onclick="setMsg('عندي شي أقوله لك، تعال! 🔥')">تعال بسرعة</button><button onclick="setMsg('اشتقت لك! كيف حالك اليوم؟ 😊')">اشتقت لك</button></div><button id="sendBtn" onclick="sendNow()">إرسال الإشعار</button><div id="result"></div><a href="/admin" class="back">⬅ العودة للوحة التحكم</a></div>
<script>
function setMsg(m){{document.getElementById('body').value=m;}}
function sendNow(){{
    var title=document.getElementById('title').value.trim()||'نبراس';
    var body=document.getElementById('body').value.trim();
    if(!body){{alert('اكتب رسالة');return;}}
    var btn=document.getElementById('sendBtn');
    btn.disabled=true;btn.textContent='جاري الإرسال...';
    fetch('/admin/send_notification',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{title:title,body:body}})}})
        .then(function(r){{return r.json();}})
        .then(function(d){{
            btn.disabled=false;btn.textContent='إرسال الإشعار';
            if(d.status==='ok'){{document.getElementById('result').innerHTML='✅ تم الإرسال إلى <b>'+d.sent+'</b> جهاز';}}
            else{{document.getElementById('result').innerHTML='❌ خطأ: '+(d.message||'فشل');}}
        }}).catch(function(){{btn.disabled=false;btn.textContent='إرسال الإشعار';document.getElementById('result').innerHTML='❌ خطأ في الاتصال';}});
}}
</script></body></html>"""

@app.route('/admin/send_notification',methods=['POST'])
def admin_send_notification():
    if not session.get('is_admin'):return jsonify({"status":"error","message":"غير مصرح"}),403
    try:
        d=request.get_json();title=d.get('title','نبراس');body=d.get('body','')
        if not body:return jsonify({"status":"error","message":"الرسالة فارغة"}),400
        sent=send_to_all(title,body)
...
def cron_daily():
    ...
    expected=os.environ.get("CRON_KEY","nibras-cron-default")
    if key!=expected:return "Unauthorized",403
    import random
    messages=["ها ما ودك نسولف اليوم؟ 💬","وين اليوم مختفي؟ 👀","عندي شي أقوله لك، تعال! 🔥","اشتقت لك! كيف حالك اليوم؟ 😊","تعال نسولف شوي ☕"]
    msg=random.choice(messages)
    sent=send_to_all("نبراس",msg)
    return jsonify({"status":"ok","sent":sent,"message":msg})
...
def chat():
    try:
        ...
        has_user=bool(session.get('user_email')) and not is_admin
        
        if has_user:
            conn=get_db();row=conn.execute("SELECT code_expiry FROM users WHERE email=?",(session['user_email'],)).fetchone();conn.close()
            if row and row['code_expiry']:
                try:
                    exp=datetime.fromisoformat(row['code_expiry'])
                    if exp.date()<datetime.now().date():
                        session.clear()
                        return jsonify({"reply":"انتهت صلاحية كودك. تواصل مع المطور.","audio":None})
                except:pass
        
        is_registered=is_admin or has_user
        is_guest=not is_registered
        uid=get_user_id()
        
        if is_guest:
            if not check_guest_limit_safe(uid):
                reply_limit="وصلت للحد المجاني اليوم (15 سؤال) 😊\n\n💡 للحصول على وصول كامل، تواصل مع المطور."
        ...
        if is_image_request(um) and not has_image:
            ...
            if is_video:
                ...
                if vr and vr.startswith("ERROR:"):
                    ...
                    sm[uid].append({"role":"user","content":um});sm[uid].append({"role":"assistant","content":rw});nid=save_user_conversation(uid,sm[uid],cid)
                    return jsonify({"reply":rw,"image_url":vr,"conv_id":nid})
                else:
                    reply="⚠️ تعذر جلب الفيديو."
                    sm[uid].append({"role":"user","content":um});sm[uid].append({"role":"assistant","content":reply});nid=save_user_conversation(uid,sm[uid],cid)
                    return jsonify({"reply":reply,"conv_id":nid})
        ...
        if has_image and not is_registered:
            reply="عذراً، تحليل الصور متاح للأعضاء المسجلين فقط.\n\n🔐 تواصل مع المطور للحصول على كود."
            if cid is None:sm[uid]=[]
            sm[uid].append({"role":"user","content":um});sm[uid].append({"role":"assistant","content":reply});nid=save_user_conversation(uid,sm[uid],cid)
            return jsonify({"reply":reply,"conv_id":nid})
        
        if cid is None:sm[uid]=[]
        model=OPENAI_MODEL;use_web=is_registered;allow_img=is_registered
        ...
        if img_data and is_registered:msgs.append({"role":"user","content":[{"type":"text","text":um or "حلل الصورة"},{"type":"image_url","image_url":{"url":img_data}}]})
        if use_web:
            try:
                fc=""
                for m in msgs:
        ...
        try:
            ...
            r=client.chat.completions.create(model=model,messages=msgs,max_completion_tokens=8000,reasoning_effort=rl)
