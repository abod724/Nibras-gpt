...
import markdown  # <-- إضافة مكتبة markdown
...
- اكتب ردك في فقرات نصية عادية متصلة، مع ترك سطر فارغ بين الفقرات.
- **ممنوع** وضع كل جملة في سطر مستقل (ممنوع الشعر).
- **ممنوع** استخدام HTML مباشرة مثل <ul>, <li>, <strong>, <p>، إلخ. استخدم Markdown فقط.
- استخدم تنسيق Markdown في ردودك حيثما كان مناسباً، مع الأمثلة التالية:
  * للعناوين: `# عنوان رئيسي`، `## عنوان فرعي`.
  * للقوائم غير المرقمة: `- نقطة` أو `* نقطة`.
  * للقوائم المرقمة: `1. نقطة`، `2. نقطة`.
  * للنص العريض: `**نص**`.
  * للنص المائل: `*نص*`.
  * للكود المضمن: `` `كود` ``.
  * لكتلة الكود: `` ```لغة\nكود\n``` ``.
...
def chat():
 try:
  ...
  # تنسيق الرد باستخدام markdown
  html_reply = markdown.markdown(reply, extensions=['fenced_code', 'tables', 'nl2br'])
  sm[uid].append({"role":"assistant","content":reply})  # نحفظ النص الأصلي (غير المنسق) في المحادثة
  nid=save_user_conversation(uid,sm[uid],cid)
  try:gender=session.get('voice_gender','male');audio=generate_speech(reply,gender)  # الصوت من النص الأصلي
