...
# ✅ بيانات الأدمن والمراجع من متغيرات البيئة
ADMIN_EMAIL=os.environ.get("ADMIN_EMAIL","abdullaha0569361@gmail.com")
REVIEWER_EMAIL=os.environ.get("REVIEWER_EMAIL","reviewer@nibrasgp.com")
...
def init_db():
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS deletion_requests (id INTEGER PRIMARY KEY AUTOINCREMENT,email TEXT,requested_at TEXT,processed INTEGER DEFAULT 0)''')
    ...
    conn.execute("INSERT OR IGNORE INTO users (email,role,voice_gender,created_at) VALUES (?, 'admin', 'male', ?)",(ADMIN_EMAIL.lower(),datetime.now().isoformat()))
    
    # ✅ حساب المراجع الدائم من متغيرات البيئة (بدون كود، بدون انتهاء)
    if REVIEWER_EMAIL and REVIEWER_PASSWORD:
        conn.execute("""INSERT OR REPLACE INTO users 
                        (email,role,voice_gender,password_hash,code_expiry,created_at)
                        VALUES (?, 'reviewer', 'male', ?, '2099-12-31T23:59:59', ?)""",
                     (REVIEWER_EMAIL.lower(),generate_password_hash(REVIEWER_PASSWORD),datetime.now().isoformat()))
        print(f"✅ حساب المراجع جاهز: {REVIEWER_EMAIL}")
...
def get_user_role(email):
    ...
    if email.lower()==REVIEWER_EMAIL.lower():return 'reviewer'
    conn=get_db();row=conn.execute("SELECT role FROM users WHERE email=?",(email.lower(),)).fetchone();conn.close()
...
def login():
    if request.method=='POST':
        ...
        # 2. المراجع (بريد + كلمة مرور فقط، بدون كود)
        if e==REVIEWER_EMAIL.lower():
            if not REVIEWER_PASSWORD:return render_template_string(LH,error="حساب المراجع غير مُعد.")
            if secrets.compare_digest(p,REVIEWER_PASSWORD):
                session.clear();session.permanent=True
...
def delete_my_account():
    ...
    # ✅ لا يمكن حذف حساب المراجع
    if email.lower()==REVIEWER_EMAIL.lower():
        return jsonify({"status":"error","message":"لا يمكن حذف هذا الحساب"}),403
    
    uid=get_user_id()
    conn=get_db()
    conn.execute("DELETE FROM conversations WHERE user_id=?",(uid,))
    conn.execute("DELETE FROM users WHERE email=? AND role!='admin' AND role!='reviewer'",(email,))
    conn.commit();conn.close()
    session.clear()
...
def request_deletion():
    ...
    try:
        ...
        conn.execute("INSERT INTO deletion_requests (email, requested_at, processed) VALUES (?, ?, 0)",
                     (email,datetime.now().isoformat()))
        conn.commit();conn.close()
        
        print(f"🗑️ طلب حذف حساب جديد (موثق): {email}")
...
def chat():
    try:
        ...
        user_role=get_user_role(session.get('user_email','')) if session.get('user_email') else None
        ...
        is_registered=is_admin or has_user or is_reviewer
    ...
    except Exception as e:print(f"❌ {e}");return jsonify({"status":"error","message":str(e)}),500
