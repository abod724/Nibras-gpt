...
- استخدم Markdown بشكل احترافي ومنظم.
- استخدم `#` للعناوين الرئيسية الكبيرة.
- استخدم `##` للعناوين الفرعية.
- استخدم `###` للعناوين الأصغر.
- استخدم `**النص**` لإبراز الكلمات والجمل المهمة.
- استخدم `*النص*` عند الحاجة للخط المائل.
- استخدم `-` للقوائم غير المرتبة.
- استخدم `1.` للقوائم المرتبة والخطوات.
- استخدم `>` للاقتباسات أو الملاحظات المهمة.
- استخدم `` `الكود` `` للمصطلحات البرمجية أو الأكواد القصيرة.
- استخدم `---` للفواصل بين الأقسام عند الحاجة.
- اترك سطراً فارغاً بين الفقرات والأقسام.
- لا تضع كل جملة في سطر مستقل بدون سبب.
- اجعل الرد مريحاً للقراءة ومنظماً مثل تطبيقات المحادثة الحديثة.
- لا تستخدم Markdown بشكل مبالغ فيه؛ استخدمه عندما يخدم وضوح الرد.
...
<head>
    ...
    <style>
        * {
        ...
        .msg.bot .bot-content strong {
            font-weight: 800;
            color: #111111;
        }

        .msg.bot .bot-content em {
            font-style: italic;
        }

        .msg.bot .bot-content ul,
        .msg.bot .bot-content ol {
            margin: 10px 0 18px 0;
            padding-right: 25px;
            line-height: 2;
        }

        .msg.bot .bot-content li {
            margin: 4px 0;
            padding-right: 3px;
        }

        .msg.bot .bot-content code {
            background: #f1f3f5;
            padding: 3px 7px;
            border-radius: 6px;
        ...
        .msg.bot .bot-content blockquote {
            margin: 14px 0;
            padding: 10px 15px;
        ...
        .input-area .send {
            ...
            justify-content: center;
            flex-shrink: 0;
            box-shadow: 0 2px 8px rgba(74,106,138,0.2);
        }
...
<div class="app">
    ...
    <div class="header">
        <div class="header-right">
            <button class="mute-btn" id="muteBtn" title="كتم الصوت / تفعيل الصوت">
    ...
    <div class="dropdown" id="dropdown">
        ...
        <button class="item" onclick="window.location.href='/plans'">
    ...
    <div class="input-area">
        ...
        <button class="btn-icon mic-btn" id="micBtn" aria-label="تسجيل صوتي">
            <i class="fas fa-microphone"></i>
        </button>

        <button class="plus-btn" id="plusBtn" aria-label="إضافة ملف">
        ...
        <button class="send" id="sendBtn" aria-label="إرسال الرسالة">
...
(function() {
    ...
    function formatBotText(text) {
        ...
        lines.forEach(function(line) {
            ...
            // العناصر التي تم تحويلها بالفعل إلى HTML
    ...
    function addMessage(text, sender, isSystem, imageData) {
        ...
        if (imageData) {
            ...
            el.innerHTML =
                '<img src="' + imageData + '" class="image-upload" />' +
                '<span class="file-label">' + (text || 'صورة') + '</span>' +
    ...
    userInput.addEventListener('input', function() {
        ...
        this.style.height =
            Math.min(this.scrollHeight, 80) + 'px';
    });
...
def load_conversation(conv_id):
    ...
    messages = load_conversation_by_id(
        user_id,
        conv_id
    )
...
def chat():
    ...
    try:
        ...
        is_admin = (
            'admin_email' in session
            and
            session['admin_email']
            == "abdullaha0569361@gmail.com"
        )
        ...
        if is_admin:
            ...
            trial_remaining > 0
            and
            not session.get('is_trial_expired')
        ):
        ...
        if (
            ...
            if image_url:
                ...
                print(
                    "⚠️ فشل توليد الصورة، نكمل للرد النصي."
                )
        ...
        if use_web_search:
            ...
            try:
                ...
                search_response = client.responses.create(
                    ...
                    input=(
                        "ابحث في الويب عن أحدث المعلومات حول: "
                        f"{user_message}، "
                        "وقدم لي ملخصاً مفيداً."
                    ),
                    tools=[
                        {
                            "type": "web_search"
                ...
                search_result = (
                    search_response.output_text.strip()
                )
        ...
        try:
            ...
            if not reply:
                ...
                reply = (
                    "ما قدرت أجيب لك رد، حاول مرة أخرى."
                )

        except openai.BadRequestError as e:

            print(
                f"⚠️ فشل نموذج {model}: {e}. "
                "جارٍ التبديل لـ gpt-4o-mini."
            )
            ...
            try:
                ...
                if not reply:
                    ...
                    reply = (
                        "فشل النموذج المتقدم، "
                        "تم التبديل للنموذج العادي."
                    )

            except Exception as e2:

                reply = (
                    "حدث خطأ في الاتصال بـ OpenAI: "
                    + str(e2)
                )
            ...
            reply = (
                "حدث خطأ في السيرفر، حاول مرة أخرى."
            )
        ...
        if (
            ...
            if session['trial_remaining'] == 0:
                ...
                reply += (
                    "\n\n⚠️ انتهت محادثاتك التجريبية. "
                    "الترقية للاستمرار مع البحث بالويب والصور."
                )

        try:

            user_gender = session.get(
                'voice_gender',
                'male'
            )
