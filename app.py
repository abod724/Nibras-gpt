...
(function(){
    // عرض أي خطأ في الشاشة
    window.onerror = function(msg, url, line, col, error) {
        var chat = document.getElementById('chat');
        if (chat) {
            var err = document.createElement('div');
            err.className = 'msg error';
            err.textContent = '⚠️ خطأ JS: ' + msg;
    ...
    try {
        ...
        if (!chat) { document.body.innerHTML += '<div style="color:red;padding:20px;">❌ #chat غير موجود</div>'; return; }
        if (!ui) { chat.innerHTML += '<div class="msg error">❌ #userInput غير موجود</div>'; return; }
        if (!sendBtn) { chat.innerHTML += '<div class="msg error">❌ #sendBtn غير موجود</div>'; return; }

        chat.innerHTML += '<div class="msg bot">✅ تم تحميل الواجهة بنجاح، الأزرار جاهزة.</div>';
        ...
        // دالة إضافة رسالة بسيطة للاختبار
        function addDebug(msg, isError) {
            var el = document.createElement('div');
            el.className = 'msg ' + (isError ? 'error' : 'bot');
            el.textContent = msg;
            chat.appendChild(el);
        ...
        // ربط زر الإرسال
        sb.onclick = function() {
            addDebug('🔘 تم الضغط على زر الإرسال!', false);
            var text = ui.value.trim();
            if (!text) { addDebug('⚠️ اكتب شيئاً أولاً', true); return; }
            // هنا سنضيف لاحقاً منطق الإرسال الفعلي، لكن أولاً نختبر الزر
            addDebug('📨 الرسالة: ' + text, false);
        };

        // ربط زر الميكروفون
        if (mb) {
            mb.onclick = function() {
                addDebug('🎤 تم الضغط على الميكروفون', false);
                if (!('webkitSpeechRecognition' in window)) {
                    addDebug('❌ المتصفح لا يدعم التعرف الصوتي', true);
                    return;
                }
                addDebug('🎤 جاري الاستماع...', false);
            };
        } else {
            addDebug('⚠️ زر الميكروفون غير موجود', true);
        }
        ...
        if (mt && dd) {
            mt.onclick = function(e) {
                ...
                addDebug('📋 تم فتح/إغلاق القائمة', false);
            };
        } else {
            addDebug('⚠️ زر القائمة أو القائمة غير موجودين', true);
        }
        ...
        if (pb && po) {
            ...
            pb.onclick = function(e) {
                ...
                addDebug('➕ تم فتح/إغلاق الإضافات', false);
            };
        } else {
            addDebug('⚠️ زر الإضافة غير موجود', true);
        }
        ...
        addDebug('✅ تم ربط جميع الأزرار بنجاح!', false);

    } catch(e) {
        var chat = document.getElementById('chat');
        if (chat) {
            chat.innerHTML += '<div class="msg error">❌ خطأ فادح: ' + e.message + '</div>';
        }
        console.error(e);
    }
})();
